...
if TYPE_CHECKING:
    ...
    # Runtime type alias for server interface
...
class ToolRegistry:
    ...
    async def load_previous_dynamic_tools(
        ...
        try:
            if server is None:
                raise ValueError(
                    "Server interface is required for load_previous_dynamic_tools"
                )
    ...
    async def generate_new_dynamic_tools(
        ...
        try:
            ...
            if server is None:
                raise ValueError(
                    "Server interface is required for generate_new_dynamic_tools"
                )
