...
from composer.models import UserConfig as UserConfigModel
from composer.models.default_model_profiles import DEFAULT_MODEL_PROFILE_CONFIG
from composer.models.default_configs import (
...
from composer.server.cache import cache_storage as cache
...
class UserConfig:
    ...
    def _memory_cache_get(self, user_id: str) -> Optional[UserConfigModel]:
    ...
    def _memory_cache_set(
        self, user_id: str, config: UserConfigModel, ttl: Optional[int] = None
    ) -> None:
    ...
    async def get_user_config(self, user_id: str) -> UserConfigModel:
    ...
    async def _get_user_config_from_database(self, user_id: str) -> UserConfigModel:
    ...
    async def update_user_config(self, user_id: str, cfg: UserConfigModel) -> None:
        ...
        try:
            ...
            try:
                cache.cache_user_config(user_id, cfg)
            except Exception as e:
                logger.warning(
    ...
    async def _update_user_config_in_database(
        self, user_id: str, cfg: UserConfigModel
    ) -> None:
                        ...
                        if "config" in user_dict and user_dict["config"]:
                            try:
                                ...
                                try:
                                    ...
                                    self._ensure_required_fields(config_dict)
                                    user_dict["config"] = UserConfigModel(
                                        **config_dict
                                    ).model_dump()
                                except Exception as e:
                                    logger.warning(
                                    ...
                                    user_dict["config"] = create_default_user_config(
                                        user_id
                                    ).model_dump()
                            except Exception as e:
                                logger.warning(
    ...
    async def ensure_user_exists(self, user_id: str) -> UserConfigModel:
        """
    ...
    async def update_user_config_in_users_table(
        self, user_id: str, config: UserConfigModel
    ) -> None:
        """
