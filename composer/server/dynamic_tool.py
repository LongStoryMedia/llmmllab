...
import json
import logging
from composer.models.dynamic_tool import DynamicTool as DynamicToolModel
...
class DynamicToolStorage:
    ...
    def _parse_tool_from_row(self, row) -> DynamicToolModel:
        ...
        return DynamicToolModel(**tool_data)

    async def get_tool_by_id(
        self, tool_id: uuid.UUID, user_id: str
    ) -> Optional[DynamicToolModel]:
    ...
    ) -> Tuple[List[DynamicToolModel], PaginationSchema]:
        """
    ...
    async def update_tool(self, tool: DynamicToolModel) -> Optional[DynamicToolModel]:
    ...
    async def search_tools_by_embedding(
        ...
        Returns:
            Tuple containing list of tools and pagination metadata
        """
        vector_str = Memory.format_embedding_for_pgvector(query_embedding)
        ...
        async with self.typed_pool.acquire() as conn:
            ...
            for row in rows:
                try:
                    ...
                    # Create the tool and add similarity score as an attribute
                    tool = DynamicToolModel(**tool_data)
                    setattr(tool, "similarity_score", similarity_score)
                    tools.append(tool)
                except Exception as e:
                    logger.error(f"Error creating DynamicTool from database: {e}")
