...
from typing import Protocol, Optional, List, Any, Callable, Tuple
...
from composer.models import (
    ...
    MessageRole,
    Summary,
    DynamicTool as DynamicToolModel,
    ModelProfileConfig,
    UserConfig as UserConfigModel,
    ModelProfile as ModelProfileModel,
)
from runner.models import ModelProfile as RunnerModelProfile
...
class ConversationService(Protocol):
    ...
    async def update_conversation_title(
        self, title: str, conversation_id: int, user_id: str
    ) -> None: ...
...
class DynamicToolService(Protocol):
    ...
    async def create_tool(self, tool: DynamicToolModel) -> DynamicToolModel: ...

    async def get_tool_by_id(self, tool_id: str) -> Optional[DynamicToolModel]: ...

    async def get_tools_by_user(self, user_id: str) -> List[DynamicToolModel]: ...

    async def list_tools_by_user(
        self, user_id: str, limit: int = 10, offset: int = 0
    ) -> Tuple[List[DynamicToolModel], Any]: ...

    async def list_all_tools(
        self, limit: int = 10, offset: int = 0
    ) -> Tuple[List[DynamicToolModel], Any]: ...

    async def update_tool(self, tool: DynamicToolModel) -> None: ...

    async def delete_tool(self, tool_id: str) -> None: ...


class SearchService(Protocol):
    """Protocol for search service."""
...
class CheckpointService(Protocol):
    """Protocol for checkpoint service."""
