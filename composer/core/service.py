...
from typing import Dict, Optional, Type, TYPE_CHECKING, cast, Any
...
if TYPE_CHECKING:
    ...
    # Runtime type alias to avoid circular imports
    # Using Any to allow flexibility at runtime while providing type hints
    ServerInterface = Any  # type: ignore
    ...
    async def compose_workflow(
        ...
        **build_kwargs,
        ...
        try:
            ...
            if self.server:
                user_config = cast(UserConfig, await self.server.user_config.get_user_config(user_id))
            else:
                raise RuntimeError("Server interface is required for compose_workflow")

            # 2. Use per-user cache if enabled (cache based on user_id only now)
            user_cache = None
            if user_config is not None and user_config.workflow.enable_workflow_caching:
