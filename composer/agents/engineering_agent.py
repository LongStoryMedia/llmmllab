...
class EngineeringAgent(BaseAgent):
    ...
    async def generate_technical_response(
        ...
        user_id: str,
        domain: Optional[TechnicalDomain] = TechnicalDomain.PROGRAMMING,
    ...
    async def generate_dynamic_tool_specification(
        ...
        Returns:
            A list of DynamicToolModel specifications.
        """
        dynamic_tools = []
        try:
            self.logger.info(
            ...
            try:
                ...
                dt = DynamicToolModel(**parsed_response)
...
Generate either a skip response or a structured tool specification in JSON format matching this schema: {json.dumps(DynamicToolModel.model_json_schema())}. Focus on practical implementation that directly addresses the user's needs."""
        return prompt
