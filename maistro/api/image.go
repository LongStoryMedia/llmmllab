...
// // Deprecated: RegisterStableDiffusionRoutes adds stable diffusion related endpoints
...
// 	if err != nil {
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, err.Error())
// 		return handleError(err, fiber.StatusInternalServerError, "Failed to create proxy request")
// 	}
...
// 	resp, err := client.Do(req)
// 	if err != nil {
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, fmt.Sprintf("Failed to contact Stable Diffusion API: %v", err))
// 		return handleError(err, fiber.StatusBadGateway, "Failed to contact Stable Diffusion API")
// 	}
// 	defer resp.Body.Close()
...
// 	if err != nil {
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, fmt.Sprintf("Failed to read Stable Diffusion API response: %v", err))
// 		return handleError(err, fiber.StatusInternalServerError, "Failed to read Stable Diffusion API response")
// 	}
...
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, fmt.Sprintf("Stable Diffusion API error: %d", resp.StatusCode))
...
// 		cc, err := pxcx.GetCachedConversation(userID, originalRequest.ConversationID)
// 		if err != nil {
// 			util.LogWarning("Failed to get conversation context", logrus.Fields{
// 				"userId": userID,
// 				"error":  err.Error(),
// 			})
// 			sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, "Failed to retrieve conversation context")
...
// 				"error":  err.Error(),
// 			})
// 			sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, "Failed to add image to conversation context")
...
// 		imageGenerationNotification := models.ImageGenerationNotification{
// 			ImageID:      &imageID,
// 			DownloadURL:  sdResponse.Download,
// 			ThumbnailURL: &sdResponse.Download,
// 			B64Data:      &sdResponse.Image,
// 			Metadata:     imageMetadata,
// 		}

// 		// Send success notification via WebSocket
// 		sendImageGenerationSuccessNotification(originalRequest.ConversationID, userID, imageGenerationNotification)
// 	}
...
// // ProxyImageGeneration proxies image generation requests to the Stable Diffusion API
// // and sends WebSocket notifications on completion
// func GenImg(ctx context.Context, userID string, originalRequest models.ImageGenerateRequest) error {
// 	// Create a modified proxyStableDiffusionRequest with WebSocket notification
// 	baseURL := getStableDiffusionBaseURL()
// 	targetURL := fmt.Sprintf("%s%s", baseURL, "/generate-image")
...
// 	if err != nil {
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, fmt.Sprintf("Failed to marshal request body: %v", err))
// 		return handleError(err, fiber.StatusInternalServerError, "Failed to marshal request body")
// 	}
...
// 		sendImageGenerationFailureNotification(originalRequest.ConversationID, userID, fmt.Sprintf("Stable Diffusion API error: %d", resp.StatusCode))
// 		return fmt.Errorf("Stable Diffusion API error: %d", resp.StatusCode)
// 	}
...
// 		cc.AddImage(ctx, imageMetadata, sdResponse.Download)
// 		if err != nil {
// 			util.LogWarning("Failed to add image to conversation context", logrus.Fields{
// 				"userId": userID,
// 				"error":  err.Error(),
...
// func sendImageGenerationFailureNotification(conversationID int, userID, errorMessage string) {
// 	ss := session.GlobalStageManager.GetSessionState(userID, conversationID)
// 	stage := ss.GetStage(models.SocketStageTypeGeneratingImage)
// 	stage.Fail(errorMessage, nil)
// }

// // sendImageGenerationSuccessNotification sends a WebSocket notification for successful image generation
// func sendImageGenerationSuccessNotification(conversationID int, userID string, payload models.ImageGenerationNotification) {
// 	ss := session.GlobalStageManager.GetSessionState(userID, conversationID)
// 	stage := ss.GetStage(models.SocketStageTypeGeneratingImage)
// 	stage.Complete("Image generation completed", payload)
// }
