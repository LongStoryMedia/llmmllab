...
This updated requirements document extends the previous refactoring plan to incorporate **streaming chat behavior**, **configurable research workflows**, and **advanced tool management**. We continue to use a *Composer* layer that centralizes LangGraph workflow construction and execution. The Composer will now handle **streaming events** (via LangGraph's astream_events API) for primary chat responses, while secondary agents and tool nodes produce non-streamed, batch results. Search-based research workflows will become *user-configurable*: search depth, source count, and full-text retrieval can be adjusted per conversation (guided by intent and the model's context length). The **ToolRegistry** will be refactored for composability and reuse: an intent analysis step will select or generate tools, existing dynamic tools will be discovered via semantic search[[1]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=To%20address%20this%2C%20you%20can,at%20runtime%20using%20semantic%20search) (to avoid duplication), and tools can be composed at runtime into higher-level operations. These enhancements ensure the system stays flexible and performant, leveraging LangGraph v1-alpha capabilities like streaming event hooks and the Model Context Protocol (MCP)[[2]](https://langchain-ai.github.io/langgraph/concepts/mcp/#:~:text=Model%20Context%20Protocol%20,adapters%60%20library).

## 1. Current State Analysis

*No changes to existing analysis; original architecture pain points remain valid.*

### 1.1 Architecture Problems Identified

* **Fragmented Orchestration**: Manual coordination between server components (title generation, summarization, retrieval) is error-prone.

* **Tool Management Complexity**: Dynamic tools are scattered and non-modular.

* **Pipeline Coupling**: Server and runner are tightly coupled via factories, hindering flexibility.

* **Inconsistent State Management**: Mixed LangGraph and manual state handling.

* **Streaming Limitations**: Primary agent outputs were streamed manually outside the core graph; secondary agents had no streaming support.

* **Resource and Error Handling Gaps**: Cleanup and circuit breakers are scattered.

### 1.2 Current Component Responsibilities

* **Server**: HTTP handlers, config, profiles, legacy orchestration (to be removed).

* **Composer**: (new) Graph construction, tool registry, workflow orchestration.

* **Runner**: Pipeline execution (stream/run/embed) with LangGraph.

* **Tools**: Static tools (search, summarization) and dynamic tools (code-generated), currently ad-hoc.

### 1.3 Key Pain Points

* Circuit-breaker and streaming config flow through layers without clear ownership.

* Dynamic tool logic is embedded in server, not centralized.

* RAG operations and retrieval are manually triggered.
...
**Key Updates:**  
- **Streaming in Composer:** All graph execution (including streaming) is handled in Composer. The **astream_events** API is used to emit streaming chunks, specifically the on_chat_model_stream event for partial LLM outputs[[3]](https://python.langchain.com/docs/concepts/streaming/#:~:text=from%20langchain_core,ChatPromptTemplate%20from%20langchain_anthropic%20import%20ChatAnthropic).  
- **Tool and Multi-Agent Updates:** Tool calls and agent handoffs use LangGraph's built-in mechanisms (ToolNode, Command objects) for composability[[4]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=1.%20,tools%20in%20parallel)[[5]](https://langchain-ai.github.io/langgraph/how-tos/multi_agent/#:~:text=2.%20The%20,agent%20graph).  
- **Configurable Retrieval:** Research/workflow nodes read user- or intent-level settings for how to perform searches and retrievals.

### 2.2 Composer Component Design

The Composer orchestrates:

* **Workflow Construction:** Build LangGraph StateGraphs based on conversation context and intent (chat, research, creative, or multi-agent)[[6]](https://docs.langchain.com/oss/javascript/langchain/overview#:~:text=started%20on%20building%20agents%20with,for%20basic%20LangChain%20agent%20usage).

* **Tool Lifecycle:** Centralize registration of static tools and manage dynamic tools with semantic search lookup[[1]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=To%20address%20this%2C%20you%20can,at%20runtime%20using%20semantic%20search).

* **Streaming Execution:** Handle the full execution of the graph, using workflow.astream_events(...) to produce intermediate events for streaming when needed[[3]](https://python.langchain.com/docs/concepts/streaming/#:~:text=from%20langchain_core,ChatPromptTemplate%20from%20langchain_anthropic%20import%20ChatAnthropic).

* **State Management:** Track conversation state, available tools, and metadata; enforce max token limits.

## 3. Detailed Requirements

### 3.1 Composer Core Modules

#### 3.1.1 Composer Service
...
* **Context Length Aware:** Before passing state to nodes (especially for research synthesis), the StateManager should ensure that the total tokens in state.messages do not exceed the LLM's context window (n_ctx). If necessary, it can drop or summarize earlier messages. This preserves coherence when retrieving and synthesizing sources.
...
### 3.4 Migration Strategy and Implementation Checklist
...
* **Metrics:** Expand ComposerMetrics to count dynamic tool generations, workflow streaming latency, and cache hits/misses.

* **Logging:** Emit structured logs for each streaming event (chunk sent), each tool execution, and agent handoffs.

### 3.6 Configuration Management

In ComposerConfig, add fields for research and search behaviors:
...
class ComposerConfig:
    ...
    # Research/workflow-specific configs
    default_search_depth: int = 2
    default_max_sources: int = 5
    enable_full_content_retrieval: bool = False
    # ... existing fields ...
```

* The conversation or user can override these per-thread. The StateManager and builder must propagate these into WorkflowState and nodes.

## 4. Key Best Practices and Notes
...
* LangChain & LangGraph Concepts: Streaming, Tool Calling, Multi-Agent, MCP[[3]](https://python.langchain.com/docs/concepts/streaming/#:~:text=from%20langchain_core,ChatPromptTemplate%20from%20langchain_anthropic%20import%20ChatAnthropic)[[4]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=1.%20,tools%20in%20parallel)[[1]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=To%20address%20this%2C%20you%20can,at%20runtime%20using%20semantic%20search)[[5]](https://langchain-ai.github.io/langgraph/how-tos/multi_agent/#:~:text=2.%20The%20,agent%20graph)[[2]](https://langchain-ai.github.io/langgraph/concepts/mcp/#:~:text=Model%20Context%20Protocol%20,adapters%60%20library)[[10]](https://docs.langchain.com/oss/javascript/langchain/overview#:~:text=Standard%20model%20interface%20Different%20providers,Learn%20more%2036%20Debug%20with). These guides inform our implementation of streaming and dynamic tool logic.

* LangChain v1-alpha docs: emphasize using astream_events and Command primitives for event streaming and multi-agent control.

* Existing LangGraph examples (ToolNode parallel calls, prebuilt agent patterns) for reference.

---

[[1]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=To%20address%20this%2C%20you%20can,at%20runtime%20using%20semantic%20search) [[4]](https://langchain-ai.github.io/langgraph/how-tos/tool-calling/#:~:text=1.%20,tools%20in%20parallel) Call tools
https://langchain-ai.github.io/langgraph/how-tos/tool-calling/

[[2]](https://langchain-ai.github.io/langgraph/concepts/mcp/#:~:text=Model%20Context%20Protocol%20,adapters%60%20library) Overview
https://langchain-ai.github.io/langgraph/concepts/mcp/

[[3]](https://python.langchain.com/docs/concepts/streaming/#:~:text=from%20langchain_core,ChatPromptTemplate%20from%20langchain_anthropic%20import%20ChatAnthropic) [[9]](https://python.langchain.com/docs/concepts/streaming/#:~:text=LangGraph%20%20compiled%20graphs%20are,support%20the%20standard%20streaming%20APIs) Streaming | ️ LangChain
https://python.langchain.com/docs/concepts/streaming/

[[5]](https://langchain-ai.github.io/langgraph/how-tos/multi_agent/#:~:text=2.%20The%20,agent%20graph) Custom implementation
https://langchain-ai.github.io/langgraph/how-tos/multi_agent/

[[6]](https://docs.langchain.com/oss/javascript/langchain/overview#:~:text=started%20on%20building%20agents%20with,for%20basic%20LangChain%20agent%20usage) [[10]](https://docs.langchain.com/oss/javascript/langchain/overview#:~:text=Standard%20model%20interface%20Different%20providers,Learn%20more%2036%20Debug%20with) Overview - Docs by LangChain
https://docs.langchain.com/oss/javascript/langchain/overview

[[7]](file://file-KGRowE6RrRi63YDxHvVDwx#:~:text=%60%60%60python%20composer%2F__init__.py%20class%20ComposerService%3A%20,composer%20service%20coordinating%20all%20subcomponents) [[8]](file://file-KGRowE6RrRi63YDxHvVDwx#:~:text=class%20ToolExecutorNode%3A%20,from%20previous%20node) refactor.md
file://file-KGRowE6RrRi63YDxHvVDwx
