...
class ProductionDynamicToolSystem:
    ...
    async def create_and_validate_tool(
        ...
        if self.marketplace and user_id:
            ...
            if marketplace_tools:
                logger.info(
                    f"Found {len(marketplace_tools)} existing tools in marketplace"
                )
        ...
        if self.marketplace and user_id:
            ...
            tags = [
                ...
                if len(word) > 4
                and word.lower() not in ("this", "that", "with", "from")
            ]

            self.marketplace.publish_tool(
                tool, user_id, tags=tags[:5]
            )  # Limit to 5 tags
    ...
    def get_execution_statistics(self):
        ...
        successful_execs = sum(
            1
            for data in self.execution_monitor.values()
        ...
        return {
            ...
            "success_rate": (
                successful_execs / len(self.execution_monitor)
