...
from models import Message, MessageRole, MessageContent, MessageContentType, Conversation
...
async def verify_api_key_access(request: Request) -> str:
    ...
    if api_key:
        ...
        if result:
            logger.debug(f"Ollama API access granted with API key for user {result.user_id} from IP {client_ip}")
            return result.user_id
        logger.warning(f"Invalid API key provided for Ollama endpoint from IP {client_ip}")
        raise HTTPException(
    ...
    if auth_header and auth_header.startswith("Bearer "):
        ...
        result = await auth_middleware.api_key_validator.validate_api_key(api_key)
        if result:
            logger.debug(f"Ollama API access granted with Bearer API key for user {result.user_id} from IP {client_ip}")
            return result.user_id
        logger.warning(f"Invalid API key provided in Bearer token for Ollama endpoint from IP {client_ip}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid or expired API key"
        )
...
def verify_local_network_access(request: Request) -> None:
    ...
    if not is_local_network(client_ip):
        ...
        raise HTTPException(
            status_code=403,
            detail="Ollama API is only accessible from local network"
        )
...
async def generate(body: OllamaGenerateRequest, user_id: str = Depends(verify_api_key_access)):
    """
    ...
    conversation_obj = Conversation(
        id=0,
        user_id=user_id,
        title=f"Generate request ({datetime.now().strftime('%Y-%m-%d %H:%M')})",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    )
    conversation_id = await storage.get_service(storage.conversation).create_conversation(conversation_obj)
    ...
    if body.stream:
        return StreamingResponse(
            stream_composer_to_ollama_generate(user_id, conversation_id, body.model),
...
@router.post("/chat")
async def chat(body: OllamaChatRequest, user_id: str = Depends(verify_api_key_access)):
    """
    ...
    conversation_obj = Conversation(
        id=0,
        user_id=user_id,
        title=f"Chat request ({datetime.now().strftime('%Y-%m-%d %H:%M')})",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    )
    conversation_id = await storage.get_service(storage.conversation).create_conversation(conversation_obj)
    ...
    if body.stream:
        return StreamingResponse(
            stream_composer_to_ollama_chat(user_id, conversation_id, body.model),
            media_type="application/x-ndjson"
        )
    else:
        raise HTTPException(status_code=501, detail="Non-streaming not yet implemented")
...
@router.post("/create")
async def create_model(body: OllamaCreateRequest, request: Request, user_id: str = Depends(verify_api_key_access)):
    """
    Create a model.
...
@router.post("/copy")
async def copy_model(body: OllamaCopyRequest, request: Request, user_id: str = Depends(verify_api_key_access)):
    """
    Copy a model.
...
async def delete_model(body: OllamaDeleteRequest, request: Request, user_id: str = Depends(verify_api_key_access)):
    """
    Delete a model.
...
@router.post("/pull")
async def pull_model(body: OllamaPullRequest, request: Request, user_id: str = Depends(verify_api_key_access)):
    """
...
@router.post("/push")
async def push_model(body: OllamaPushRequest, request: Request, user_id: str = Depends(verify_api_key_access)):
    """
