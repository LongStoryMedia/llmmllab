...
import json
from typing import AsyncIterator, List, Literal, Optional, Union, Dict, Any
...
class OpenAIChatMessage(BaseModel):
    ...
    content: Optional[Union[str, List[Dict[str, Any]]]] = (
        None  # String or array of content parts
    )
...
def convert_openai_to_internal_messages(
    openai_messages: List[OpenAIChatMessage], conversation_id: int
) -> List[Message]:
    ...
    for msg in openai_messages:
        ...
        if isinstance(msg.content, str):
            content.append(
                MessageContent(type=MessageContentType.TEXT, text=msg.content)
            )
            ...
            for item in msg.content:
                if isinstance(item, dict):
                    if item.get("type") == "text":
                        content.append(
                            MessageContent(
                                type=MessageContentType.TEXT, text=item.get("text", "")
                            )
                        )
                        ...
                        content.append(
                            MessageContent(type=MessageContentType.IMAGE, image_url=url)
                        )

        # If no content was extracted, add empty text
        if not content:
            content.append(MessageContent(type=MessageContentType.TEXT, text=""))
...
async def stream_composer_to_openai_chat(
    user_id: str, conversation_id: int, request_id: str, model_name: str
) -> AsyncIterator[str]:
...
async def create_chat_completion(body: OpenAIChatCompletionRequest, request: Request):
    ...
    from models.conversation import Conversation as _Conversation

    conversation_obj = _Conversation(
        id=0,
        user_id=user_id,
        title=f"Local conversation ({datetime.now().strftime('%Y-%m-%d %H:%M')})",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    )
    conversation_id = await storage.get_service(
        storage.conversation
    ).create_conversation(conversation_obj)

    # Convert and store all messages
    internal_messages = convert_openai_to_internal_messages(
    ...
    if body.stream:
        return StreamingResponse(
            stream_composer_to_openai_chat(
                user_id, conversation_id, str(uuid4()), body.model
            ),
...
async def create_completion(body: OpenAICompletionRequest, request: Request):
    ...
    if isinstance(body.prompt, str):
        ...
        if body.prompt and isinstance(body.prompt[0], (list, int)):
            ...
            raise HTTPException(
                status_code=400, detail="Token ID prompts not supported"
            )
        prompts = body.prompt
    else:
    ...
    if len(prompts) > 1:
        raise HTTPException(
            status_code=400, detail="Multiple prompts not yet supported"
        )

    # Create conversation and message
    conversation = await storage.get_service(storage.conversation).create_conversation(
        user_id
    )
    ...
    if body.stream:
        ...
        raise HTTPException(
            status_code=501, detail="Streaming completions not yet implemented"
        )
    else:
        # TODO: Implement non-streaming response
        raise HTTPException(
            status_code=501, detail="Non-streaming completions not yet implemented"
        )
