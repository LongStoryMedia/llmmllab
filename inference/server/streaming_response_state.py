...
class StreamingResponseState:
    ...
    def process_chunk(self, chunk: str) -> ChatResponse:
        ...
        elif self.state == StreamingState.PROCESSING:
            return self._handle_processing_content(chunk)
        elif self.state == StreamingState.EXECUTING:
    ...
    def _check_state_transitions(self, chunk: str) -> None:
        ...
        if self.tool_call_end_pattern.search(chunk):
            if self.current_tool_call:
                ...
                try:
                    tool_data = json.loads(self.tool_call_buffer.strip())
                    self.current_tool_call["tool_name"] = tool_data.get("name", "")
                    self.current_tool_call["args"] = tool_data.get("args", tool_data.get("arguments", {}))
    ...
    def _handle_responding_content(self, chunk: str) -> ChatResponse:
        ...
        content = [MessageContent(type=MessageContentType.TEXT, text=clean_chunk)] if clean_chunk else []
        return ChatResponse(
            message=Message(
                role=MessageRole.ASSISTANT,
                content=content
    ...
    def _clean_xml_tags(self, chunk: str) -> str:
        """Remove XML tags and JSON metadata from chunk."""
        if not chunk:
            return chunk
        ...
        # Filter out JSON metadata patterns
        json_pattern = r'\{"[^"]*":\s*\[?\{[^}]*\}[^}]*\]?\}'
        chunk = re.sub(json_pattern, '', chunk)
    ...
    def get_final_response(self) -> ChatResponse:
        ...
        Returns:
            Final ChatResponse with all content properly organized
        """
        content = [MessageContent(type=MessageContentType.TEXT, text=self.response_buffer)] if self.response_buffer else []
        thinking = Thought(text=self.accumulated_thinking) if self.accumulated_thinking else None
