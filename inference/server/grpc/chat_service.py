...
class ChatService(inference_pb2_grpc.InferenceServiceServicer):
        ...
        try:
            ...
            if isinstance(pipeline, dict):
                ...
                if model is None or tokenizer is None:
                    raise ValueError(
                        f"Pipeline for {model_id} missing model or tokenizer"
                    )
    ...
    def _role_proto_to_model(
        self, role: message_role_pb2.MessageRole.ValueType
    ) -> MessageRole:
        """
    ...
    def _content_type_proto_to_model(
        self, content_type: message_content_type_pb2.MessageContentType.ValueType
    ) -> MessageContentType:
        """
    ...
    def _content_proto_to_model(
        self, content: message_content_pb2.MessageContent
    ) -> MessageContent:
        """
    ...
    def _options_proto_to_model(
        self, options: model_parameters_pb2.ModelParameters
    ) -> ModelParameters:
        """
    ...
    def _role_model_to_proto(
        self, role: MessageRole
    ) -> message_role_pb2.MessageRole.ValueType:
        """
    ...
    def _content_type_model_to_proto(
        self, content_type: MessageContentType
    ) -> message_content_type_pb2.MessageContentType.ValueType:
        """
    ...
    def _content_model_to_proto(
        self, content: MessageContent
    ) -> message_content_pb2.MessageContent:
        """
    ...
    def _finish_reason_model_to_proto(
        self, finish_reason: str
    ) -> chat_response_pb2.ChatResponse.FinishReasonEnum.ValueType:
        """
    ...
    def _chat_response_model_to_proto(
        self, response: ChatResponse
    ) -> chat_response_pb2.ChatResponse:
        """
        ...
        """
        message = (
            self._message_model_to_proto(response.message) if response.message else None
        )
        created_at = None
        ...
        return chat_response_pb2.ChatResponse(
            ...
            finish_reason=self._finish_reason_model_to_proto(
                response.finish_reason if response.finish_reason else "stop"
            ),
            created_at=created_at,
            ...
            prompt_eval_count=(
                response.prompt_eval_count if response.prompt_eval_count else 0.0
            ),
            prompt_eval_duration=(
                response.prompt_eval_duration if response.prompt_eval_duration else 0.0
            ),
    ...
    def ChatStream(self, request: chat_req_pb2.ChatReq, context):
        ...
        self.logger.info(
            f"Chat request received, conversation {request.conversation_id}"
        )
        self.logger.info(
            f"Original messages: {[(m.role, len(m.content)) for m in request.messages]}"
        )
        ...
        self.logger.info(
            f"Fixed messages: {len(fixed_messages)} messages after processing"
        )

        if not fixed_messages:
            error_content = message_content_pb2.MessageContent(
                type=message_content_type_pb2.TEXT,
                text="I'm sorry, there was an error processing your request.",
            )
            error_response = chat_response_pb2.ChatResponse(
                model=model_id,
                message=message_pb2.Message(
                    role=message_role_pb2.MessageRole.ASSISTANT, content=[error_content]
                ),
                done=True,
        ...
        try:
            ...
            error_content = message_content_pb2.MessageContent(
                text="I'm sorry, I'm having trouble loading my thinking capabilities right now."
            )
            error_response = chat_response_pb2.ChatResponse(
                model=model_id,
                message=message_pb2.Message(
    ...
    def _fix_message_sequence(
        self, messages: List[message_pb2.Message]
    ) -> List[message_pb2.Message]:
        """
        ...
        if not messages:
            return []
        expected_role_order = [
            message_role_pb2.MessageRole.SYSTEM,
            message_role_pb2.MessageRole.USER,
            message_role_pb2.MessageRole.ASSISTANT,
            message_role_pb2.MessageRole.USER,
            message_role_pb2.MessageRole.ASSISTANT,
        ]  # And so on
        fixed_messages = []
        ...
        for i, msg in enumerate(messages):
            ...
            if role == current_role:
                ...
                extracted_text = ""
                if isinstance(current_content, list) or hasattr(
                    current_content, "__iter__"
                ):
                    for content_item in current_content:
                        if isinstance(
                            content_item, message_content_pb2.MessageContent
                ...
                new_msg = message_pb2.Message(
                    ...
                    conversation_id=(
                        msg.conversation_id if hasattr(msg, "conversation_id") else -1
                    ),
                    id=(
                        msg.id if i == 0 and hasattr(msg, "id") else -1
                    ),  # Use first message's ID or default
                )
            ...
            new_msg = message_pb2.Message(
                ...
                conversation_id=(
                    messages[-1].conversation_id
                    if messages and hasattr(messages[-1], "conversation_id")
                    else -1
                ),
                id=-1,  # Default ID
            )
        ...
        if (
            fixed_messages
            and fixed_messages[0].role == message_role_pb2.MessageRole.SYSTEM
        ):
        ...
        for msg in fixed_messages:
            ...
            if role == message_role_pb2.MessageRole.SYSTEM and result_messages:
                self.logger.warning(
                    f"Skipping additional system message: {msg.content[:30]}..."
                )
                continue
            ...
            if not result_messages or role == expected_role_order[expected_role_index]:
                ...
                expected_role_index = (expected_role_index + 1) % len(
                    expected_role_order
                )
                ...
                self.logger.warning(
                    f"Skipping message with unexpected role {role}, expected {expected_role_order[expected_role_index]}"
                )

        # Ensure the sequence ends with a user message for the model to respond as assistant
        if (
            result_messages
            and result_messages[-1].role == message_role_pb2.MessageRole.ASSISTANT
        ):
            self.logger.warning(
                "Removing last assistant message to maintain proper sequence"
            )
            result_messages.pop()

        self.logger.info(
            f"Fixed message sequence: {[(msg.role, len(msg.content)) for msg in result_messages]}"
        )
