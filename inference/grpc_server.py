...
import protos.chat_req_pb2
import protos.chat_req_pb2
import protos.chat_req_pb2
import protos.chat_response_pb2
import protos.embedding_req_pb2
import protos.embedding_response_pb2
import protos.generate_req_pb2
import protos.generate_response_pb2
import protos.image_generation_request_pb2
import protos.image_generation_response_pb2
...
from config import logger, HF_HOME, IMAGE_RETENTION_HOURS, get_grpc_config
import os
import sys
import argparse
import logging
...
class InferenceServicer(InferenceServiceServicer):
    ...
    def __init__(self):
        ...
        self.logger.info(f"Services initialized: chat={self.chat_service is not None}, image={self.image_service is not None}, embedding={self.embedding_service is not None}")
    ...
    def ChatStream(self, request, context):
        ...
        """
        self.logger.info(f"Received ChatStream request with {len(request.messages)} messages")
...
def serve(port=50051):
    ...
    if protos.inference_pb2_grpc:
        try:
            ...
            service_methods = dir(protos.inference_pb2_grpc)
            print(f"Available methods in inference_pb2_grpc: {[m for m in service_methods if not m.startswith('_')]}")

            # Try the standard way first
            if hasattr(protos.inference_pb2_grpc, 'add_InferenceServiceServicer_to_server'):
                print("Using standard method to register service")
                protos.inference_pb2_grpc.add_InferenceServiceServicer_to_server(servicer, server)  # type: ignore
            ...
            # Try direct method binding as fallback
                ...
                method_handlers = {
                    'ChatStream': grpc.stream_stream_rpc_method_handler(
                        servicer.ChatStream,
                        request_deserializer=protos.chat_req_pb2.ChatReq.FromString,
                        response_serializer=protos.chat_response_pb2.ChatResponse.SerializeToString,
                    ),
                    'GenerateStream': grpc.stream_stream_rpc_method_handler(
                        servicer.GenerateStream,
                        request_deserializer=protos.generate_req_pb2.GenerateReq.FromString,
                        response_serializer=protos.generate_response_pb2.GenerateResponse.SerializeToString,
                    ),
                    'GetEmbedding': grpc.unary_unary_rpc_method_handler(
                        servicer.GetEmbedding,
                        request_deserializer=protos.embedding_req_pb2.EmbeddingReq.FromString,
                        response_serializer=protos.embedding_response_pb2.EmbeddingResponse.SerializeToString,
                    ),
                    'GenerateImage': grpc.unary_unary_rpc_method_handler(
                        servicer.GenerateImage,
                        request_deserializer=protos.image_generation_request_pb2.ImageGenerateRequest.FromString,
                        response_serializer=protos.image_generation_response_pb2.ImageGenerateResponse.SerializeToString,
                    ),
                ...
                server.add_generic_rpc_handlers((
                    grpc.method_handlers_generic_handler('inference.InferenceService', method_handlers),
                ))
            ...
            try:
                print(f"inference_pb2 available: {protos.inference_pb2 is not None}")
                print(f"inference_pb2_grpc available: {protos.inference_pb2_grpc is not None}")

                if protos.inference_pb2_grpc:
                    print("Service class inspection:")
                    service_classes = [obj for obj in dir(protos.inference_pb2_grpc) if not obj.startswith('_') and 'Service' in obj]
