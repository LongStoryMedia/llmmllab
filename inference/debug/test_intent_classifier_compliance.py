...
def create_test_conversation(user_query: str) -> ConversationCtx:
    ...
    conversation = Conversation(
        id="test_conv_001", title="Test Conversation", messages=[message]
    )
...
async def test_intent_analysis():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            print(
                f"     - Required Capabilities: {[cap.value for cap in intent_analysis.required_capabilities]}"
            )
            print(
                f"     - Computational Requirements: {[req.value for req in intent_analysis.computational_requirements]}"
            )
            print(
                f"     - Domain Specificity: {intent_analysis.domain_specificity:.2f}"
            )
            print(
                f"     - Reusability Potential: {intent_analysis.reusability_potential:.2f}"
            )
            ...
            if intent_analysis.primary_intent == test_case["expected_intent"]:
                print(
                    f"  ✅ Intent Classification: Expected '{test_case['expected_intent']}', got '{intent_analysis.primary_intent}'"
                )
            else:
                print(
                    f"  ⚠️  Intent Classification: Expected '{test_case['expected_intent']}', got '{intent_analysis.primary_intent}'"
                )

            if intent_analysis.complexity_level == test_case["expected_complexity"]:
                print(
                    f"  ✅ Complexity Assessment: Expected '{test_case['expected_complexity'].value}', got '{intent_analysis.complexity_level.value}'"
                )
            else:
                print(
                    f"  ⚠️  Complexity Assessment: Expected '{test_case['expected_complexity'].value}', got '{intent_analysis.complexity_level.value}'"
                )

            # Validate architectural requirements
            print(f"  ✅ Architecture Compliance:")
            print(
                f"     - Follows User Request → IntentAnalysis → RequiredCapabilities pipeline"
            )
