...
def test_pipeline_imports():
    ...
    pipeline_imports = [
        ...
        ("LlamaChatSummPipe", "runner.pipelines.txt2txt.llamachatsum", "LlamaChatSummPipe"),
        ("OpenAiGptOssPipe", "runner.pipelines.txt2txt.openai_gpt_oss", "OpenAIGptOssPipeline"),
        ("Qwen25VLGGUFPipeline", "runner.pipelines.imgtxt2txt.qwen25_vl", "Qwen25VLPipeline"),
        ("BaseLlamaCppPipeline", "runner.pipelines.llamacpp.base_llamacpp", "BaseLlamaCppPipeline"),
    ...
    for pipeline_name, module_path, class_name in pipeline_imports:
        ...
        try:
            ...
            logger.info(f"✅ {pipeline_name}: Successfully imported {class_name} from {module_path}")
            
            # Check if it has BaseChatModel methods (for chat models)
            if class_name.endswith('Pipeline') or class_name == 'Qwen3Moe' or class_name == 'LlamaChatSummPipe':
                if hasattr(pipeline_class, '_generate') and hasattr(pipeline_class, '_stream'):
                    logger.info(f"✅ {pipeline_name}: Has BaseChatModel interface methods")
                    results[pipeline_name] = "SUCCESS - BaseChatModel interface"
                else:
                    logger.warning(f"⚠️  {pipeline_name}: Missing _generate/_stream methods")
            ...
            # Check if it has Embeddings methods (for embedding models)
                ...
                if hasattr(pipeline_class, 'embed_documents') and hasattr(pipeline_class, 'embed_query'):
                    logger.info(f"✅ {pipeline_name}: Has Embeddings interface methods")
                    results[pipeline_name] = "SUCCESS - Embeddings interface"
                else:
                    logger.warning(f"⚠️  {pipeline_name}: Missing embed_documents/embed_query methods")
