...
def print_summary(flags, executable_path):
    ...
    bool_flags = sum(1 for f in flags if f.get('type') is None and f.get('action') == 'store_true')
    ...
    print(f"   Flag types: {bool_flags} bool, {int_flags} int, {float_flags} float, {str_flags} str")
...
def main():
    ...
    try:
        ...
        if not flags:
            print("❌ No flags discovered. Check if the executable exists and provides --help output.")
            return 1
