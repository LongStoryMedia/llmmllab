...
def test_direct_state_transformation():
    ...
    print(
        f"   - conversation_id: {simulated_tools_state.get('conversation_id', 'NOT_FOUND')}"
    )
    print(
        f"   - user_config present: {simulated_tools_state.get('user_config') is not None}"
    )
    ...
    print(
        f"   - not tools_state.get('user_id'): {not simulated_tools_state.get('user_id')}"
    )
