...
def test_transform_with_user_id_scenarios():
    ...
    for scenario_name, user_id_value in test_scenarios:
        ...
        try:
            ...
            memory_tool_would_pass = bool(
                user_id_from_state and user_id_from_state != ""
            )
            ...
            results.append(
                (
                    scenario_name,
                    True,
                    memory_tool_would_pass,
                    tools_state.get("user_id"),
                )
            )
...
async def test_memory_tool_with_improved_errors():
    ...
    test_cases = [
        (
            "empty_string_user_id",
            {"user_id": "", "conversation_id": 42, "user_config": None},
        ),
        ("none_user_id", {"user_id": None, "conversation_id": 42, "user_config": None}),
        (
            "missing_user_id",
            {"conversation_id": 42, "user_config": None},
        ),  # No user_id key
        (
            "valid_user_id",
            {"user_id": "test-user-123", "conversation_id": 42, "user_config": None},
        ),
    ]
    ...
    for test_name, test_state in test_cases:
        ...
        try:
            ...
            result = await memory_retrieval.ainvoke(
                {"query": "test query", "runtime": runtime}
            )
...
def main():
    ...
    for scenario, transform_ok, memory_ok, user_id in transform_results:
        ...
        print(
            f"   {status} {scenario}: transform={transform_ok}, memory_check={memory_ok}, user_id={repr(user_id)}"
        )
