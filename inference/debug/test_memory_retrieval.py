...
from models import (
    Memory, MemoryFragment, MemorySource, MessageRole, 
    ModelProfile, ModelProfileType, NodeMetadata,
    ConversationCtx, Message, UserConfig
)
...
class TestMemoryRetrieval:
    ...
    async def create_test_user(self):
        ...
        try:
            async with storage.pool.acquire() as conn:
                ...
                test_messages = [
                    (101, self.test_conversation_1, "user", "How do I install Python packages using pip?"),
                    (102, self.test_conversation_1, "assistant", "You can install Python packages using pip by running 'pip install package-name' in your terminal."),
                    (201, self.test_conversation_2, "user", "What's the best way to install Python libraries?"),
                    (202, self.test_conversation_2, "assistant", "The recommended way is to use pip, the Python package installer. Just run 'pip install library-name'.")
                ]
        ...
        try:
            ...
            if storage.pool:
                async with storage.pool.acquire() as conn:
                    ...
                    await conn.execute("DELETE FROM messages WHERE id IN (101, 102, 201, 202)")
                    
                    # Delete test conversations  
                    await conn.execute("DELETE FROM conversations WHERE id IN ($1, $2)", 
                                     self.test_conversation_1, self.test_conversation_2)
    ...
    def create_similar_embeddings(self, base_text: str, variation_text: str) -> tuple[List[float], List[float]]:
        """
    ...
    async def test_memory_search_with_agent(self):
        ...
        for memory in memories:
            ...
            assert memory.similarity >= 0.7, f"Memory similarity {memory.similarity} below threshold"
