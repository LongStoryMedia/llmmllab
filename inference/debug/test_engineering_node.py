...
async def test_engineering_node():
    ...
    try:
        ...
        test_message = Message(
            ...
            content=[
                MessageContent(type=MessageContentType.TEXT, text=test_query, url=None)
            ],
        )
        ...
        tool_storage = DynamicToolStorage(
            storage.pool, None
        )  # get_query function not needed for test
        ...
        if hasattr(result, "messages"):
            ...
            logger.info(
                f"🔍 Original messages: {original_count}, Result messages: {result_count}"
            )

            if result_count > original_count:
                new_messages = result.messages[original_count:]
                logger.info(
                    f"🔍 Found {len(new_messages)} new messages from engineering node"
                )
                ...
                for i, msg in enumerate(result.messages):
                    logger.info(
                        f"  Message {i}: type={msg.type}, content_length={len(msg.content)}"
                    )
                    ...
                    if msg.type == "ai" and len(msg.content) > 100:
                        ...
                        if (
                            "hello" in content
                            and "here to assist" in content
                            and len(content) < 1000
                        ):
                            logger.warning(
                                "⚠️  Response appears to be generic greeting despite 764 chars"
                            )
                        else:
                            logger.info("✅ Response appears to be technical content")
                        return
                ...
                if len(result.messages) == len(test_state.messages):
                    for i, (orig, res) in enumerate(
                        zip(test_state.messages, result.messages)
                    ):
