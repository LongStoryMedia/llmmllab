...
async def test_real_composer_workflow_user_id():
    ...
    try:
        ...
        print(
            f"   - messages count: {len(initial_state.messages) if initial_state.messages else 0}"
        )
        ...
        try:
            ...
            user_config = await storage.get_service(
                storage.user_config
            ).get_user_config(test_user_id)
            if not user_config:
                print(
                    f"   ⚠️ No user config found for user_id='{test_user_id}', using defaults"
                )
            ...
            print(
                f"   - ToolsState user_id: '{tools_state.get('user_id', 'NOT_FOUND')}'"
            )
            print(
                f"   - ToolsState user_id type: {type(tools_state.get('user_id', 'NOT_FOUND'))}"
            )
            print(
                f"   - ToolsState user_id is truthy: {bool(tools_state.get('user_id', ''))}"
            )
            print(
                f"   - conversation_id: {tools_state.get('conversation_id', 'NOT_FOUND')}"
            )
            ...
            print(
                f"   - not tools_state.get('user_id'): {not tools_state.get('user_id')}"
            )
