...
# Module imports
from models import (
    Memory, MemoryFragment, MemorySource, MessageRole, UserConfig
)
...
class MockMemoryStorage:
    ...
    async def search_similarity(
        ...
        relevant_memories = [
            memory for memory in self.stored_memories 
...
class TestMemoryRetrievalTool:
    ...
    async def test_memory_retrieval_tool_with_results(self):
        ...
        try:
            ...
            result = await memory_retrieval.ainvoke({"query": search_query, "runtime": runtime})
    ...
    async def test_memory_retrieval_tool_no_results(self):
        ...
        try:
            ...
            assert isinstance(result, str), "Tool should return string result"
            assert "No relevant memories found" in result, "Should indicate no memories found"
    ...
    async def test_memory_retrieval_tool_error_handling(self):
        ...
        mock_storage_service = MagicMock()
        mock_storage_service.search_similarity = AsyncMock(side_effect=Exception("Storage error"))
        ...
        try:
            ...
            assert isinstance(result, str), "Tool should return string result"
            assert "Memory retrieval failed" in result or "❌" in result, "Should indicate failure"
    ...
    async def test_memory_similarity_logic(self):
        ...
        assert len(high_threshold_results) == 2, f"Expected 2 results, got {len(high_threshold_results)}"
        assert all(m.similarity >= 0.8 for m in high_threshold_results), "All results should meet threshold"
        ...
        assert len(low_threshold_results) == 3, f"Expected 3 results, got {len(low_threshold_results)}"
        ...
        assert len(limited_results) == 1, f"Expected 1 result, got {len(limited_results)}"
        assert limited_results[0].similarity == 0.85, "Should return highest similarity memory"
    ...
    async def run_all_tests(self):
        ...
        logger.info(f"🎉 Memory Retrieval Tool Tests Complete: {passed_tests}/{total_tests} passed")
...
async def main():
    ...
    print("="*60)
    print(f"Overall Result: {results['passed_tests']}/{results['total_tests']} tests passed")
