...
def test_llamacpp_grammar_parameter():
    ...
    try:
        ...
        logger.warning("⚠️ ChatLlamaCpp does not support 'grammar' or 'grammar_path' parameters")
...
def main():
    ...
    if grammar_supported or response_format_supported:
        ...
        logger.info("ℹ️ Grammar is supported at llama-cpp-python level but not exposed in ChatLlamaCpp")
    else:
        logger.warning("⚠️ No grammar support detected - may need to use alternative approach")
    
    logger.info("🔧 Recommendation: Use response_format parameter if available, otherwise extend ChatLlamaCpp")
