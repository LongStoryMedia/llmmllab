...
def test_tool_call_parsing():
    ...
    try:
        ai_message_dict = AIMessage(
            content="I'll search for that.", tool_calls=[dict_tool_call]
        )

        logger.info(
            f"✅ AIMessage created successfully with tool_calls: {ai_message_dict.tool_calls}"
        )
        ...
        for i, tc in enumerate(extracted_dict):
            logger.info(
                f"Dict tool call {i}: name={tc.name}, id={tc.execution_id}, args={tc.args}"
            )
    ...
    try:
        ...
        logger.info(
            f"✅ Extracted {len(extracted_requests)} requests from OpenAI format"
        )
        ...
        for i, tc in enumerate(extracted_models):
            logger.info(
                f"Model {i}: name={tc.name}, id={tc.execution_id}, args={tc.args}"
            )
