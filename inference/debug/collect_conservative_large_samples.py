...
def estimate_memory_conservative(config: ConservativeTestConfig) -> float:
    ...
    logger.info(f"Conservative estimate for {config.model_name} @ {config.context_size//1024}K: {total:.1f}GB")
    return total
...
def run_conservative_test(config: ConservativeTestConfig) -> Optional[Dict]:
    ...
    try:
        ...
        cmd = [
            "kubectl", "exec", "-i", "-n", "ollama", pod_name, "--",
            "llama-server",
            ...
            "--threads", "16",  # Reduced threads
            "--no-kv-offload",
            "--cache-type-k", "f16",
            "--cache-type-v", "f16",
            "--numa", "distribute",
            "--split-mode", "layer",
            "--tensor-split", "0.25,0.45,0.30",  # More conservative split
            "--main-gpu", "1",
            "--port", "8080",
