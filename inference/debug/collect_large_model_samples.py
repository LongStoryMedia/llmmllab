...
class LargeModelSampleCollector:
    ...
    def _get_memory_estimate_from_container(self, config: LargeModelTestConfiguration) -> float:
    ...
    def _run_large_model_test(self, config: LargeModelTestConfiguration) -> LargeModelMemoryResult:
    ...
    def _save_large_model_samples(self):
        ...
        print(f"✅ Merged {len(successful_samples)} large model samples with {len(existing_samples)} existing samples")
    ...
    def _generate_large_model_summary(self):
        ...
        if successful_results:
            ...
            for result in successful_results:
                ...
                print(f"{result.config.model_name[:24] + mmproj_indicator:<28} {ctx_str:>8} {result.config.batch_size:>8} {result.estimated_gb:>8.1f} {result.total_actual_gb:>8.1f} {result.accuracy_ratio:>8.2f}")
        ...
        if failed_results:
            ...
            for result in failed_results:
                print(f"- {result.config.model_name} @ {result.config.context_size//1024}K: {result.error_msg[:100]}")
