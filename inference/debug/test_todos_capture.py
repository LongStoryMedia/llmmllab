...
async def main():
    profile = ModelProfile(
        model_name="gpt-4o-mini", system_prompt="You are a helpful assistant."
    )
    node_meta = NodeMetadata(
        node_id="debug-node",
        node_name="debug_todos",
        node_type="test",
        user_id="debug-user",
        conversation_id=123,
    )
    ...
    agent = ChatAgent(
        pipeline_factory=pipeline_factory, profile=profile, node_metadata=node_meta
    )
    ...
    messages = [
        Message(
            ...
            content=[{"text": "Plan a 5-step process to refactor legacy module"}],
        )
    ]
