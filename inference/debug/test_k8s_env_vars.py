...
def test_composer_env_vars():
    ...
    workflow_vars = {
        ...
        "COMPOSER_MAX_CONTEXT_LENGTH": lambda x: x.isdigit() and 1000 <= int(x) <= 1000000,
        "COMPOSER_CONTEXT_TRIM_THRESHOLD": lambda x: is_float(x) and 0.1 <= float(x) <= 1.0,
    ...
    tool_vars = {
        "COMPOSER_TOOL_SIMILARITY_THRESHOLD": lambda x: is_float(x) and 0.1 <= float(x) <= 1.0,
        "COMPOSER_TOOL_MODIFICATION_THRESHOLD": lambda x: is_float(x) and 0.1 <= float(x) <= 1.0,
