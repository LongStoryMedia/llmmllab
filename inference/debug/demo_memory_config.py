...
async def demo_memory_usage():
    ...
    print(
        f"   Cross-Conversation: {default_tool.memory_config.enable_cross_conversation}"
    )
    ...
    print(
        f"   Cross-Conversation: {custom_tool.memory_config.enable_cross_conversation}"
    )
    ...
    print(
        f"   Focused Tool - Threshold: {focused_tool.memory_config.similarity_threshold}, "
    ...
    print(
        f"   Broad Tool - Threshold: {broad_tool.memory_config.similarity_threshold}, "
    ...
    print(
        f"   Research Tool - Threshold: {research_tool.memory_config.similarity_threshold}, "
...
def demo_validation_and_constraints():
    ...
    try:
        ...
        print(
            f"      Limit: {valid_config.limit}, Threshold: {valid_config.similarity_threshold}"
        )
    ...
    try:
        invalid_threshold = MemoryConfig(
            **DEFAULT_MEMORY_CONFIG.model_dump(), similarity_threshold=1.5
        )
    ...
    try:
        invalid_timeout = MemoryConfig(
            **DEFAULT_MEMORY_CONFIG.model_dump(), timeout=120.0
        )
