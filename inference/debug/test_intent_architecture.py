...
try:
    ...
    checks = {
        ...
        "Current user message assertion": "current_user_message is not None"
        in agent_source,
        "User config assertion": "user_config is not None" in agent_source,
        "Pipeline factory import": "from runner import pipeline_factory"
        in agent_source,
        ...
        "Removed old extract method": "_extract_user_query" not in agent_source
        or "def _extract_user_query" not in agent_source,
    }
    ...
    try:
        ...
        capability_sample = [
            RequiredCapability.WEB_SEARCH.value,
            RequiredCapability.REASONING.value,
        ]
        print(f"  ✅ RequiredCapability sample: {capability_sample}")

        requirement_sample = [
            ComputationalRequirement.HIGH_MEMORY.value,
            ComputationalRequirement.GPU_ACCELERATION.value,
        ]
