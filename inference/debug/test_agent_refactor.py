...
def test_base_agent_create_agent():
    ...
    try:
        ...
        if 'stream_pipeline' in stream_source:
            ...
            logger.warning("⚠️  BaseAgent.stream doesn't use create_agent() (might be async issue)")
        ...
        if 'run_pipeline' in run_source:
            ...
            logger.warning("⚠️  BaseAgent.run doesn't use create_agent() (might be async issue)")
        ...
        logger.info("✅ BaseAgent architecture successfully updated to use create_agent() pattern")
        return True
...
def test_factories_available():
    ...
    try:
        ...
        if hasattr(embedding_model_factory, 'create_embedding_model'):
            ...
            logger.error("❌ EmbeddingModelFactory missing create_embedding_model method")
            return False
...
if __name__ == "__main__":
    ...
    if overall_success:
        ...
        logger.info("   - BaseAgent uses create_agent() pattern instead of legacy pipeline calls")
