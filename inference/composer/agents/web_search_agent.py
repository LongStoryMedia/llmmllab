"""
Web Search Agent for web content retrieval and processing.
...
class WebSearchAgent:
    ...
    async def _perform_shallow_search(
        ...
        try:
            ...
            # Perform quick search (limited crawling, faster results)
            search_results = await asyncio.wait_for(
                search_service.search_and_extract(
                    query=query,
                    max_results=max_results,
                    max_depth=1,  # Shallow crawl depth
    ...
    async def _perform_deep_search(
        ...
        try:
            ...
            # Perform comprehensive search with deeper crawling
            search_results = await asyncio.wait_for(
                search_service.search_and_extract(
                    query=query,
                    max_results=max_results * 2,  # Get more initial results for filtering
                    max_depth=3,  # Deeper crawl depth
                    extract_content=True,
                    follow_links=True,  # Follow relevant links
            ...
            filtered_results = self._filter_deep_search_results(search_results or [], max_results)
