...
class WorkflowState(BaseModel):
    ...
    messages: Annotated[List[LangChainMessage], add_messages] = Field(
        ...
        description="Conversation history and LLM outputs"
    )
    ...
    conversation_id: Optional[str] = Field(
        default=None,
        description="Conversation identifier for state persistence"
    )
...
class ChatWorkflowState(WorkflowState):
    ...
    title_generated: bool = Field(
        default=False,
        description="Whether conversation title has been generated"
    )
    
    tool_calls_pending: bool = Field(
        default=False,
        description="Whether there are pending tool calls to execute"
    )
...
class ResearchWorkflowState(WorkflowState):
    ...
    query_expanded: bool = Field(
        default=False,
        description="Whether initial query has been expanded"
    )
    
    sources_gathered: List[str] = Field(
        default_factory=list,
        description="List of information sources discovered"
    )
    
    synthesis_complete: bool = Field(
        default=False,
        description="Whether research synthesis is complete"
    )
...
class MultiAgentWorkflowState(WorkflowState):
    ...
    active_agent: Optional[str] = Field(
        default=None,
        description="Currently active agent identifier"
    )
