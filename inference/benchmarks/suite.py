...
def safe_import_benchmark(module_name: str, class_name: str, benchmark_key: str):
    ...
    try:
        module = importlib.import_module(
            f".academic.{module_name}", package="benchmarks"
        )
                ...
                for benchmark_name in benchmarks_to_run:
                    if benchmark_name in self.benchmarks:
                        ...
                        if benchmark_name == "humaneval":
                            samples = max(10, num_samples // 4)
                        elif benchmark_name in ["gpqa_diamond", "math_500", "ifeval"]:
                            samples = max(20, num_samples // 2)
                        elif benchmark_name == "livebench":
                            samples = max(3, num_samples // 10)
                        else:
                            samples = num_samples
                        ...
                        result = self.benchmarks[benchmark_name].run(model_id, samples)
