...
class EmbeddingService:
    ...
    def get_embedding(self, request: inference_pb2.EmbeddingRequest, context):
        """
        Generate embeddings for the provided text.
        """
        try:
            # Get the model name, using default if not provided
            model_name = request.model_name if request.model_name else self.default_embedding_model

            # Load the model if needed
            model, tokenizer = self._load_model_if_needed(model_name)

            # Log the request
            self.logger.info(f"Embedding request from user {request.user_id}, conversation {request.conversation_id}")

            # Process the text and generate embedding
            if model_name.startswith("intfloat/e5") or model_name.startswith("intfloat/multilingual-e5"):
