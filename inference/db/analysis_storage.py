    ...
    async def add_analysis(
        ...
        try:
            async with self.typed_pool.acquire() as conn:
                ...
                required_capabilities_json = json.dumps([cap.value if hasattr(cap, 'value') else str(cap) for cap in intent_analysis.required_capabilities])
                computational_requirements_json = json.dumps(intent_analysis.computational_requirements.dict())
                
                row = await conn.fetchrow(
                    self.get_query("analysis.add_analysis"),
                    message_id,                                         # $1
                    intent_analysis.workflow_type.value if hasattr(intent_analysis.workflow_type, 'value') else str(intent_analysis.workflow_type),  # $2
                    intent_analysis.complexity_level.value if hasattr(intent_analysis.complexity_level, 'value') else str(intent_analysis.complexity_level),  # $3
                    ...
                    intent_analysis.response_format.value if intent_analysis.response_format and hasattr(intent_analysis.response_format, 'value') else str(intent_analysis.response_format) if intent_analysis.response_format else None,  # $8
                    intent_analysis.technical_domain.value if intent_analysis.technical_domain and hasattr(intent_analysis.technical_domain, 'value') else str(intent_analysis.technical_domain) if intent_analysis.technical_domain else None,  # $9
                ...
                if row:
                    analysis_id = row["id"]
                    self.logger.info(f"Added analysis {analysis_id} ({intent_analysis.workflow_type}) for message {message_id}")
                    return analysis_id
                else:
    ...
    async def add_analysis_legacy(
        ...
        try:
            ...
            intent_analysis = IntentAnalysis(
                ...
                computational_requirements=analysis_data.get('computational_requirements', {})
            )
            
            return await self.add_analysis(message_id, intent_analysis, created_at)
            
        except Exception as e:
            self.logger.error(f"Error converting legacy analysis for message {message_id}: {e}")
            return None
        ...
        try:
            async with self.typed_pool.acquire() as conn:
                ...
                for row in rows:
                    ...
                    required_capabilities = row["required_capabilities"] if row["required_capabilities"] else []
                    computational_requirements = row["computational_requirements"] if row["computational_requirements"] else {}
    ...
    async def get_analyses_by_message_legacy(self, message_id: int) -> List[dict]:
        ...
        try:
            ...
            for ia in intent_analyses:
                analysis = {
                    "workflow_type": ia.workflow_type.value if hasattr(ia.workflow_type, 'value') else str(ia.workflow_type),
                    "complexity_level": ia.complexity_level.value if hasattr(ia.complexity_level, 'value') else str(ia.complexity_level),
                    "required_capabilities": [cap.value if hasattr(cap, 'value') else str(cap) for cap in ia.required_capabilities],
                    "domain_specificity": ia.domain_specificity,
                    "reusability_potential": ia.reusability_potential,
                    "confidence": ia.confidence,
                    "response_format": ia.response_format.value if ia.response_format and hasattr(ia.response_format, 'value') else str(ia.response_format) if ia.response_format else None,
                    "technical_domain": ia.technical_domain.value if ia.technical_domain and hasattr(ia.technical_domain, 'value') else str(ia.technical_domain) if ia.technical_domain else None,
                    ...
                    "computational_requirements": ia.computational_requirements.dict() if hasattr(ia.computational_requirements, 'dict') else ia.computational_requirements
                }
            ...
            self.logger.error(f"Error retrieving legacy analyses for message {message_id}: {e}")
            return []
