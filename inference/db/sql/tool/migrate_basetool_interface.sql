...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE table_name = 'dynamic_tools' AND column_name = 'args_schema'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN args_schema jsonb;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'return_direct'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN return_direct boolean DEFAULT false;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'tags'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN tags text[];
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'metadata'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN metadata jsonb DEFAULT '{}';
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'handle_tool_error'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN handle_tool_error text;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'handle_validation_error'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN handle_validation_error text;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'dynamic_tools' AND column_name = 'response_format'
    ) THEN
        ALTER TABLE dynamic_tools ADD COLUMN response_format text DEFAULT 'content';
        
        -- Add check constraint for valid response formats
        ALTER TABLE dynamic_tools ADD CONSTRAINT chk_response_format 
        CHECK (response_format IN ('content', 'content_and_artifact'));
