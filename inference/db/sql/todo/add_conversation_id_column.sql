...
BEGIN
    IF NOT EXISTS (
        ...
        BEGIN
            PERFORM decompress_chunk(c.schema_name|| '.' || c.table_name)
            FROM timescaledb_information.chunks c
            INNER JOIN timescaledb_information.hypertables h ON h.hypertable_name = c.hypertable_name
            WHERE h.hypertable_name = 'todos' AND c.is_compressed = 't';
        EXCEPTION WHEN OTHERS THEN
        ...
        ALTER TABLE todos ADD COLUMN conversation_id integer;
