...
SELECT create_hypertable('thoughts', 'created_at', if_not_exists => TRUE, chunk_time_interval => interval '3 days');
...
SELECT add_compression_policy('thoughts', INTERVAL '7 days', if_not_exists => TRUE);

-- Add retention policy for thoughts data (365 days)
SELECT add_retention_policy('thoughts', INTERVAL '365 days', if_not_exists => TRUE);