...
SELECT create_hypertable('tool_calls', 'created_at', if_not_exists => TRUE, chunk_time_interval => interval '3 days');
...
CREATE INDEX IF NOT EXISTS idx_tool_calls_data ON tool_calls USING GIN (tool_data);
...
SELECT add_retention_policy('tool_calls', INTERVAL '365 days', if_not_exists => TRUE);