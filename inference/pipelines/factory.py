...
class PipelineFactory:
    ...
    def _load_available_models(self):
        ...
        try:
            if not os.path.exists(config.MODELS_CONFIG_PATH):
                self.logger.error(
                    f"Models config file not found: {config.MODELS_CONFIG_PATH}")
                return
        ...
        self.logger.info(f"Loaded {len(self._available_models)} models into available models dictionary")
    ...
    def get_pipeline(self, model_id: str) -> Tuple[BasePipeline, float]:
        ...
        with self._cleanup_lock:
            if model_id in self._pipelines:
                ...
                return (self._pipelines[model_id].pipeline, (end_time - start_time).total_seconds() * 1000)
        ...
        if not self._available_models:
            self.logger.error("Available models dictionary is empty. Trying to reload from model service.")
        ...
        if model_id not in self._available_models:
            raise RuntimeError(f"Model with ID '{model_id}' not found in available models.")
    ...
    def create_pipeline(self, model: Model) -> Optional[BasePipeline]:
        ...
        if model.task.endswith("TextToText"):
            if model.pipeline == "GLM4VPipeline":
                ...
                self.logger.info(f"Creating Qwen 2.5 VL GGUF pipeline for model {model.name}")
                from .imgtxt2txt.qwen25_vl_gguf import Qwen25VLGGUFPipe
                pipe = Qwen25VLGGUFPipe(model)
            elif model.pipeline == "Mixtral8x7bInstructPipe":
                self.logger.info(f"Creating Mixtral-8x7B-Instruct pipeline for model {model.name}")
                from .txt2txt.mx8x7bi import Mixtral8x7bInstructPipe
                pipe = Mixtral8x7bInstructPipe(model)
            elif model.pipeline == "MixtralGGUFPipe":
                try:
                    self.logger.info(f"Creating Mixtral GGUF pipeline for model {model.name}")
                    from .txt2txt.mixtral_gguf import MixtralGGUFPipe
                    pipe = MixtralGGUFPipe(model)
                except Exception as e:
                    self.logger.warning(f"Failed to load MixtralGGUFPipe: {str(e)}")
                    self.logger.info("Falling back to MixtralTransformersPipe")
                    from .txt2txt.mixtral_transformers import MixtralTransformersPipe
                    pipe = MixtralTransformersPipe(model)
            elif model.pipeline == "Qwen30A3BPipe":
                self.logger.info(f"Creating Qwen3-30B-A3B pipeline for model {model.name}")
                from .txt2txt.qwen30a3b import Qwen30A3BPipe
                pipe = Qwen30A3BPipe(model)
            ...
            if model.pipeline == "StableDiffusionXLImg2ImgPipeline":
                self.logger.info(f"Creating SDXL Img2Img pipeline for model {model.name}")
                from .img2img.sdxl import SDXLRefinerPipe
                pipe = SDXLRefinerPipe(model)
            elif model.pipeline == "FluxKontextPipeline":
                self.logger.info(f"Creating FluxKontext pipeline for model {model.name}")
        ...
        if pipe is None:
            self.logger.error(f"Unsupported pipeline type '{model.pipeline}' for model {model.name}")
            return None
    ...
    def get_available_models(self) -> Dict[str, Model]:
        ...
        if not self._available_models:
            ...
            logger.warning("Available models dictionary is empty. Trying to reload from model service.")
            # Try to load models if dictionary is empty
    ...
    def _start_cleanup_thread(self):
        ...
        if self._cleanup_thread is None or not self._cleanup_thread.is_alive():
            self._cleanup_thread = threading.Thread(
                target=self._cleanup_cache_task,
                daemon=True
            )
    ...
    def _cleanup_expired_entries(self) -> None:
        ...
        with self._cleanup_lock:
            ...
            for model_id in models_to_remove:
                logger.info(f"Removing expired pipeline for model {model_id} from cache")
                pipe_entry = self._pipelines.pop(model_id, None)                # Clean up the pipeline's resources
    ...
    def _cleanup_pipeline_resources(self, pipeline: "BasePipeline") -> None:
        ...
        try:
            ...
            if pipeline is not None:
                self.logger.debug(f"Calling __del__ method on {type(pipeline).__name__}")
                pipeline.__del__()
        except Exception as e:
    ...
    def clear_cache(self, model_id: Optional[str] = None) -> None:
        ...
        with self._cleanup_lock:
            if model_id is not None:
                ...
                logger.info(f"Manually removing pipeline for model {model_id} from cache")
                pipe_entry = self._pipelines.pop(model_id, None)                # Clean up the pipeline's resources
                if pipe_entry and pipe_entry.pipeline:
                    self._cleanup_pipeline_resources(pipe_entry.pipeline)
            else:
