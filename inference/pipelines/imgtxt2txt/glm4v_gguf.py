...
class GLM4VGGUFPipe(BasePipeline):
    ...
    def __init__(self, model_definition: Model):
        ...
        self.model = Llama(
            ...
            model_path=self.model_def.details.gguf_file if self.model_def.details.gguf_file else self.model_def.model,
    ...
    def _format_message_content(self, content: MessageContent) -> Dict[str, Any]:
        ...
        """
        if content.type == MessageContentType.IMAGE and hasattr(content, 'url') and content.url:
    ...
    def run(self, req: ChatReq, load_time: float) -> Generator[ChatResponse, Any, None]:
        """
        Run the GLM-4.1V GGUF model to process multimodal messages.
        ...
        top_p = req.options.top_p if req.options and req.options.top_p is not None else 0.95
        top_k = req.options.top_k if req.options and req.options.top_k is not None else 40
        max_tokens = req.options.num_predict if req.options and req.options.num_predict is not None else 1024
        ...
        try:
            ...
            for chunk in completion:
                ...
                if isinstance(chunk, dict) and "choices" in chunk and len(chunk["choices"]) > 0:
                    ...
                    if "delta" in choice and "content" in choice["delta"]:
                        ...
                        response = ChatResponse(
                            ...
                            eval_duration=(
                                current_time - prompt_eval_end).total_seconds() * 1000,
                ...
                # Handle completion
                if isinstance(chunk, dict) and "choices" in chunk and len(chunk["choices"]) > 0 and chunk["choices"][0].get("finish_reason"):
