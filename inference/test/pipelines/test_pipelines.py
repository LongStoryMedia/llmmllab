...
class TestSD3Pipe:
    ...
    def test_load_sd3_pipeline(
        self, mock_get_dtype, mock_transformer, mock_sd3_pipeline, sd3_model
    ):
        ...
        with patch(
            "pipelines.sd3.SD3Pipe._setup_quantization_config", return_value=bnb_config
        ):
    ...
    @patch("pipelines.sd3.SD3Transformer2DModel")
    @patch("pipelines.sd3.get_dtype")
    def test_load_sd3_pipeline_no_quantization(
        self, mock_get_dtype, mock_transformer, mock_sd3_pipeline, sd3_model
    ):
        ...
        with patch(
            "pipelines.sd3.SD3Pipe._setup_quantization_config", return_value=None
        ):
...
class TestSDXLPipe:
    ...
    def test_load_sdxl_pipeline(
        self, mock_get_precision, mock_get_dtype, mock_sdxl_pipeline, sdxl_model
    ):
...
class TestFluxPipe:
    ...
    def test_load_flux_pipeline(self, mock_transformer, mock_flux_pipeline, flux_model):
        ...
        # Call the method under test with patch to avoid actual loading
        with patch(
            "pipelines.flux.FluxPipe._setup_quantization_config",
            return_value=bnb_config,
        ):
