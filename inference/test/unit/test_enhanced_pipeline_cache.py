...
class TestEnhancedPipelineCache:
    ...
    def test_eviction_order_simulation(self, mock_entry_class, cache_manager):
        ...
        entries = [
            Mock(
                priority=PipelinePriority.HIGH, estimated_memory=0.4e9, access_count=50
            ),  # Small, frequent
            Mock(
                priority=PipelinePriority.NORMAL, estimated_memory=3e9, access_count=10
            ),  # Medium
            Mock(
                priority=PipelinePriority.HIGH, estimated_memory=8e9, access_count=2
            ),  # Large, rare
            Mock(
                priority=PipelinePriority.LOW, estimated_memory=15e9, access_count=1
            ),  # Large, very rare
        ]

        # Set up mock eviction scores (higher = better, kept longer)
        entries[0].eviction_score.return_value = (
            100  # Best score - small, frequent, high priority
        )
        entries[1].eviction_score.return_value = (
            50  # Good score - medium size, decent usage
        )
        entries[2].eviction_score.return_value = 20  # Lower score - large, rarely used
        entries[3].eviction_score.return_value = (
            10  # Worst score - large, very rare, low priority
        )
        ...
        assert (
            sorted_entries[0] == entries[3]
        )  # Large, low priority should be evicted first
        assert (
            sorted_entries[-1] == entries[0]
        )  # Small, high priority should be kept longest
    ...
    def test_priority_based_scoring(self):
        ...
        high_priority_entry = _PipelineCacheEntry(
            Mock(), PipelinePriority.HIGH, same_size
        )
        normal_priority_entry = _PipelineCacheEntry(
            Mock(), PipelinePriority.NORMAL, same_size
        )
        low_priority_entry = _PipelineCacheEntry(
            Mock(), PipelinePriority.LOW, same_size
        )
    ...
    def test_memory_estimation_accuracy_against_real_data(
        self, cache_manager, real_memory_samples
    ):
        ...
        if len(successful_samples) < 5:
            pytest.skip(
                f"Need at least 5 successful samples, got {len(successful_samples)}"
            )
        ...
        for sample in test_samples:
            ...
            estimated_gb = cache_manager.estimate_memory(mock_model, mock_profile) / (
                1024**3
            )
        ...
        ratios = [
            r["accuracy_ratio"] for r in accuracy_results if r["accuracy_ratio"] > 0
        ]
        ...
        if len(ratios) >= 5:
            ...
            for result in accuracy_results[:5]:  # Show first 5
                print(
                    f"  {result['model']} @ {result['context']//1024}K: "
    ...
    def test_cache_corrected_memory_estimation(self, cache_manager):
        ...
        breakdown = cache_manager._calculate_corrected_memory_breakdown(
            test_params, test_model
        )
        ...
        assert (
            abs(breakdown["total_gpu_gb"] - expected_total) < 0.01
        ), f"Total {breakdown['total_gpu_gb']:.2f}GB should equal sum of components {expected_total:.2f}GB"

    def test_cache_size_categorization_with_real_data(
        self, cache_manager, real_memory_samples
    ):
        ...
        if small_models:
            ...
            assert (
                small_estimate < 10 * 1024**3
            ), f"Small model estimate {small_estimate/(1024**3):.1f}GB seems high"
        ...
        if large_models:
            ...
            assert (
                large_estimate > 15 * 1024**3
            ), f"Large model estimate {large_estimate/(1024**3):.1f}GB seems low"
        ...
        for sample in real_memory_samples[:5]:  # Test first 5 samples
            ...
            if actual_gb > 0:
                ...
                if memory_total > cache_manager._max_cache_size_gb:
                    print(
                        f"Would trigger eviction after {added_models} models, total: {memory_total:.1f}GB"
                    )
                    break
    ...
    def test_cache_performance_with_real_model_sizes(
        self, cache_manager, real_memory_samples
    ):
        ...
        assert (
            recommended_cache_size < 200
        ), "Recommended cache size should be realistic (< 200GB)"
    ...
    def test_memory_estimation_edge_cases_from_real_data(
        self, cache_manager, real_memory_samples
    ):
        ...
        if vision_samples:
            ...
            assert (
                vision_estimate > mock_model.size
            ), "Vision model estimate should include CLIP overhead"
