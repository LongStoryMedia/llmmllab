...
class TestResizerRealWorldValidation:
    ...
    def real_memory_samples(self) -> List[Dict[str, Any]]:
        ...
        samples_path = os.path.join(
            os.path.dirname(__file__), "real_memory_samples.json"
        )
    ...
    def _create_mock_model_from_sample(self, sample: Dict[str, Any]) -> Model:
        ...
        clip_model_size = (
            1024 * 1024 * 1024 if sample.get("mmproj_path") else 0
        )  # 1GB estimate
        ...
        model_details = ModelDetails(
            ...
            hidden_size=self._estimate_hidden_size_from_param_size(
                sample["param_size"]
            ),
            n_heads=64,
            n_kv_heads=8,
    ...
    def _create_optimal_parameters_from_sample(
    ...
    def test_resizer_accuracy_against_real_samples(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        assert (
            len(test_samples) >= 10
        ), f"Need at least 10 successful samples, got {len(test_samples)}"
        ...
        assert (
            0.1 <= avg_accuracy <= 10.0
        ), f"Average accuracy {avg_accuracy:.2f} outside reasonable range"
    ...
    def test_resizer_with_small_models(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        small_model_samples = [
            ...
            and any(
                size in sample["param_size"].upper()
                for size in ["2B", "3B", "3.2B", "4B"]
            )
        ]

        assert (
            len(small_model_samples) >= 5
        ), f"Need at least 5 small model samples, got {len(small_model_samples)}"
        ...
        for result in accuracy_results:
            print(
                f"  {result['model']} ({result['param_size']}) @ {result['context']//1024}K: "
        ...
        assert (
            0.2 <= avg_accuracy <= 8.0
        ), f"Small model average accuracy {avg_accuracy:.2f} outside expected range"

    def test_resizer_with_large_models(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        large_model_samples = [
            ...
            for sample in real_memory_samples
            if sample.get("total_actual_gb", 0) > 0
            and any(size in sample["param_size"].upper() for size in ["30B", "32B"])
        ]

        if len(large_model_samples) < 3:
            pytest.skip(
                f"Not enough large model samples for testing, got {len(large_model_samples)}"
            )
        ...
        assert (
            0.1 <= avg_accuracy <= 15.0
        ), f"Large model average accuracy {avg_accuracy:.2f} outside expected range"

    def test_resizer_with_vision_models(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        vision_model_samples = [
            ...
            for sample in real_memory_samples
            if sample.get("total_actual_gb", 0) > 0
            and sample.get("mmproj_path") is not None
        ]

        if len(vision_model_samples) < 2:
            pytest.skip(
                f"Not enough vision model samples for testing, got {len(vision_model_samples)}"
            )
        ...
        for sample in vision_model_samples:
            ...
            assert (
                breakdown["clip_model_gb"] > 0
            ), "Vision model should have CLIP model component"
        ...
        assert (
            0.1 <= avg_accuracy <= 15.0
        ), f"Vision model average accuracy {avg_accuracy:.2f} outside expected range"

    def test_resizer_context_scaling(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        for result in context_results:
            print(
                f"  {result['context_size']//1024}K context: "
        ...
        for i in range(1, len(context_results)):
            ...
            assert (
                curr["context_size"] > prev["context_size"]
            ), "Results should be ordered by context size"
            assert (
                curr["kv_cache_gb"] > prev["kv_cache_gb"]
            ), "KV cache should increase with context size"
            # Total memory should generally increase (allowing for some variance)
            assert (
                curr["estimated_gb"] >= prev["estimated_gb"] * 0.9
            ), "Total memory should generally increase with context"

    def test_resizer_memory_breakdown_components(
        self, resizer: Resizer, real_memory_samples: List[Dict[str, Any]]
    ):
        ...
        test_samples = [
            sample
            for sample in real_memory_samples[:10]  # First 10 samples
            if sample.get("total_actual_gb", 0) > 0
        ]
        ...
        for sample in test_samples:
            ...
            assert (
                breakdown["model_weights_gpu_gb"] >= 0
            ), "Model weights should be non-negative"
            assert breakdown["kv_cache_gb"] >= 0, "KV cache should be non-negative"
            assert (
                breakdown["activation_gb"] >= 0
            ), "Activation memory should be non-negative"
            ...
            assert (
                breakdown["model_weights_gpu_gb"] > 0
            ), "Model weights should be present"

            # For models with mmproj, verify CLIP component
            if sample.get("mmproj_path"):
                assert (
                    breakdown["clip_model_gb"] > 0
                ), "Vision models should have CLIP component"
            else:
                assert (
                    breakdown["clip_model_gb"] == 0
                ), "Text-only models should not have CLIP component"
    ...
    def test_resizer_specific_assertions_from_real_data(self, resizer: Resizer):
        ...
        assert (
            2.5 <= breakdown_4k["total_gpu_gb"] <= 8.0
        ...
        assert (
            breakdown_128k["total_gpu_gb"] > breakdown_4k["total_gpu_gb"]
        ), "128K context should require more memory than 4K context"

        assert (
            breakdown_128k["kv_cache_gb"] > 2.0
        ...
        assert (
            15.0 <= breakdown_32b["total_gpu_gb"] <= 35.0
        ...
        assert (
            breakdown_32b["clip_model_gb"] > 0
        ), "32B VL model should have CLIP component"

        # Model weights should be substantial
        assert (
            breakdown_32b["model_weights_gpu_gb"] >= 15.0
