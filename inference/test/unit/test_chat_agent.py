...
class TestChatAgent:
    ...
    async def test_chat_completion_non_streaming(self, mock_run_pipeline):
        ...
        mock_pipeline = Mock()
        pipeline_factory.pipeline.return_value.__enter__ = Mock(return_value=mock_pipeline)
    ...
    @patch('composer.agents.chat_agent.message_to_langchain_message')
    @patch('runner.run_pipeline')
    async def test_chat_completion_with_conversion(self, mock_run_pipeline, mock_convert):
