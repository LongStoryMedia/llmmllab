...
class TestModelLoader:
    ...
    def test_all_models_have_required_fields(self):
        ...
        for model_id, model in models.items():
            ...
            assert model.modified_at is not None, f"Model {model_id} modified_at is None"
            ...
            assert model.details.size is not None, f"Model {model_id} missing details.size"
            assert model.details.size >= 0, f"Model {model_id} has negative size: {model.details.size}"
            assert model.details.original_ctx is not None, f"Model {model_id} missing details.original_ctx"
            assert model.details.original_ctx > 0, f"Model {model_id} has invalid original_ctx: {model.details.original_ctx}"
            ...
            assert model.details.parameter_size, f"Model {model_id} missing details.parameter_size"
    ...
    def test_model_types_have_appropriate_details(self):
        ...
        print(f"Found {len(llama_cpp_models)} llama_cpp models and {len(hf_models)} HF models")
        ...
        for model_id, model in llama_cpp_models:
            ...
            if details.n_layers:
                assert details.n_layers > 0, f"GGUF model {model_id} has invalid layer count"
        ...
        for model_id, model in hf_models:
            ...
            assert details.parameter_size, f"HF model {model_id} should have parameter_size"
    ...
    def test_model_details_field_types(self):
        ...
        for model_id, model in models.items():
            ...
            # Test field types
            assert isinstance(details.size, (int, float)), f"Model {model_id} size is not numeric: {type(details.size)}"
            assert isinstance(details.original_ctx, int), f"Model {model_id} original_ctx is not int: {type(details.original_ctx)}"
            assert isinstance(details.format, str), f"Model {model_id} format is not string: {type(details.format)}"
            assert isinstance(details.family, str), f"Model {model_id} family is not string: {type(details.family)}"
            assert isinstance(details.families, list), f"Model {model_id} families is not list: {type(details.families)}"
            assert isinstance(details.parameter_size, str), f"Model {model_id} parameter_size is not string: {type(details.parameter_size)}"
            
            # Test optional fields when present
            if details.specialization:
                assert isinstance(details.specialization, str), f"Model {model_id} specialization is not string: {type(details.specialization)}"
                # Validate specialization values
                valid_specializations = ["Text", "LoRA", "Embedding", "TextToImage", "ImageToImage", "Audio"]
                assert details.specialization in valid_specializations, f"Model {model_id} has invalid specialization: {details.specialization}"
            
            if details.dtype:
                assert isinstance(details.dtype, str), f"Model {model_id} dtype is not string: {type(details.dtype)}"
            
            if details.weight is not None:
                assert isinstance(details.weight, (int, float)), f"Model {model_id} weight is not numeric: {type(details.weight)}"
    ...
    def test_model_architectural_details(self):
        ...
        for model_id, model in models.items():
            ...
            if has_arch_info:
                ...
                if details.layers:
                    assert details.layers > 0, f"Model {model_id} has invalid layers: {details.layers}"
                if details.heads:
                    assert details.heads > 0, f"Model {model_id} has invalid heads: {details.heads}"
                if details.ctx_length:
                    assert details.ctx_length > 0, f"Model {model_id} has invalid ctx_length: {details.ctx_length}"
                if details.vocab_size:
                    assert details.vocab_size > 0, f"Model {model_id} has invalid vocab_size: {details.vocab_size}"
        
        # We expect most models to have architectural details from GGUF metadata
        print(f"Models with architectural details: {models_with_arch_details}/{len(models)}")
    ...
    def test_lora_weights_loading(self):
        ...
        for model_id, model in models.items():
            ...
            assert isinstance(model.lora_weights, list), f"Model {model_id} lora_weights is not a list"
            
            for lora in model.lora_weights:
                assert isinstance(lora, LoraWeight), f"Model {model_id} has invalid LoRA weight type"
    ...
    def test_model_statistics(self):
        ...
        assert isinstance(stats["specializations"], dict), "Specializations should be a dictionary"
        ...
        assert isinstance(stats["models_with_clip"], int), "CLIP count should be an integer"
        assert isinstance(stats["models_with_lora"], int), "LoRA count should be an integer"
        assert isinstance(stats["average_size_gb"], (int, float)), "Average size should be numeric"
        ...
        assert stats["size_range"]["max"] >= stats["size_range"]["min"], "Max size should be >= min size"
    ...
    def test_get_model_by_id(self):
        ...
        if models:
            ...
            assert retrieved_model is not None, f"Could not retrieve model {first_model_id}"
    ...
    def test_dynamic_field_mapping(self):
        ...
        expected_details_fields = ["size", "original_ctx", "format", "family", "specialization"]
    ...
    def test_reload_models_functionality(self):
        ...
        assert reloaded_count == initial_count, "Model count should be consistent after reload"
        
        # Verify model IDs are the same
        initial_ids = set(initial_models.keys())
        reloaded_ids = set(reloaded_models.keys())
        assert initial_ids == reloaded_ids, "Model IDs should be consistent after reload"
