...
class TestSearchService:
    ...
    @patch("server.services.search_service.SearchProviderFactory")
    @patch("server.services.search_service.storage")
    async def test_real_world_ai_search(self, mock_storage, mock_factory, mock_user_config):
        ...
        with patch("server.services.search_service.pipeline_factory", mock_pipeline_factory):
            search_service = SearchService(mock_user_config)
            ...
            for content in result.contents:
                assert any(term in content.title.lower() + content.content.lower() 
    ...
    @patch("server.services.search_service.SearchProviderFactory") 
    @patch("server.services.search_service.storage")
    async def test_result_limit_enforcement(self, mock_storage, mock_factory, mock_user_config):
    ...
    @patch("server.services.search_service.SearchProviderFactory")
    @patch("server.services.search_service.storage")
    async def test_content_quality_validation(self, mock_storage, mock_factory, mock_user_config):
        ...
        with patch("server.services.search_service.pipeline_factory", mock_pipeline_factory):
            ...
            urls = [content.url for content in result.contents]
            assert any("noaa.gov" in url for url in urls), "Should include NOAA as authoritative source"
            assert any("nasa.gov" in url for url in urls), "Should include NASA as authoritative source"
            
            # Check content relevance and quality
            for content in result.contents:
                assert content.relevance >= 0.85, f"Content should be highly relevant: {content.relevance}"
                content_text = (content.title + " " + content.content).lower()
                assert "climate" in content_text, "Content should mention climate"
                assert any(term in content_text for term in ["data", "temperature", "change"]), \
            ...
            assert relevances == sorted(relevances, reverse=True), "Results should be ordered by relevance"
            
            assert result.error is None

    @patch("server.services.search_service.SearchProviderFactory")
    @patch("server.services.search_service.storage")
    async def test_query_formatting_optimization(self, mock_storage, mock_factory, mock_user_config):
        ...
        mock_pipeline = MagicMock()
        mock_pipeline.get.return_value = "cybersecurity threats 2024 ransomware"  # Optimized query
        mock_pipeline_factory = MagicMock()
        mock_pipeline_factory.get_pipeline.return_value = (mock_pipeline, None)
        ...
        with patch("server.services.search_service.pipeline_factory", mock_pipeline_factory):
            ...
            assert isinstance(result, SearchResult)
            assert result.query == "cybersecurity threats 2024 ransomware"  # Should use formatted query
