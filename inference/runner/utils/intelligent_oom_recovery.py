...
class IntelligentOOMRecovery:
    ...
    def __init__(self, data_dir: str = "/tmp/oom_recovery_data"):
        ...
        if HAS_SKLEARN:
            ...
            self.logger.warning("scikit-learn not available, using fallback heuristics")
    ...
    def _extract_features(self, model_size_mb: float, target_n_ctx: int, 
                         available_gpu_memory_mb: float, requested_gpu_layers: int) -> np.ndarray:
        ...
        return np.array([
            ...
            model_size_mb / available_gpu_memory_mb,  # Memory pressure ratio
            target_n_ctx / 1000,  # Context size in thousands
            np.log(model_size_mb + 1),  # Log model size for non-linear relationship
        ])
    
    def predict_optimal_parameters(self, model_path: str, target_n_ctx: int, 
                                 target_batch: int, target_ubatch: int,
                                 requested_gpu_layers: int, hardware_manager) -> Dict[str, int]:
        """
        ...
        features = self._extract_features(model_size_mb, target_n_ctx, 
                                        available_gpu_memory_mb, requested_gpu_layers)
        
        if HAS_SKLEARN and all(model is not None for model in self.models.values()):
            try:
                ...
                for param_name, model in self.models.items():
                    if model is not None:
                        ...
                        pred_value = self.scalers[param_name].inverse_transform([[pred_scaled]])[0][0]
                        predictions[param_name] = max(int(pred_value), 1)
                    else:
                        predictions[param_name] = self._get_fallback_value(param_name, 
                                                                          target_n_ctx, target_batch, target_ubatch, requested_gpu_layers)
                
                self.logger.info(f"ML predicted parameters: {predictions}")
                return predictions
                
            except Exception as e:
                self.logger.warning(f"ML prediction failed, using heuristics: {e}")
        
        # Fallback to heuristic-based prediction
        return self._heuristic_prediction(model_size_mb, target_n_ctx, target_batch, 
                                        target_ubatch, requested_gpu_layers, available_gpu_memory_mb)
    
    def _get_fallback_value(self, param_name: str, target_n_ctx: int, target_batch: int, 
                           target_ubatch: int, requested_gpu_layers: int) -> int:
    ...
    def _heuristic_prediction(self, model_size_mb: float, target_n_ctx: int, 
                            target_batch: int, target_ubatch: int, requested_gpu_layers: int,
                            available_gpu_memory_mb: float) -> Dict[str, int]:
    ...
    def execute_recovery_strategy(self, attempt: int, original_params: Dict[str, int],
                                current_params: Dict[str, int], hardware_manager) -> Tuple[Dict[str, int], str]:
        """
        ...
        if attempt <= 2:
            ...
            new_params['n_batch'] = max(current_params['n_batch'] // reduction_factor, 32)
            new_params['n_ubatch'] = max(current_params['n_ubatch'] // reduction_factor, 32)
            ...
            if original_gpu_layers > 0:
                ...
                layers_to_move = min(max_cpu_layers, (attempt - 4) * 5)  # Progressive movement
                new_params['n_gpu_layers'] = max(original_gpu_layers - layers_to_move, 0)
            else:
        ...
        self.logger.info(f"OOM recovery attempt {attempt}: strategy={strategy}, params={new_params}")
        return new_params, strategy
    
    def record_success(self, model_path: str, params: Dict[str, int], 
                      hardware_manager, initialization_time_ms: float = 0,
        ...
        if len(self.configurations) % 10 == 0 and HAS_SKLEARN:
            self._train_models()
    
    def record_failure(self, attempt: int, strategy: str, params: Dict[str, int], error_message: str):
    ...
    def _train_models(self):
        ...
        if not HAS_SKLEARN or len(self.configurations) < 10:
            return
        ...
        try:
            ...
            for config in self.configurations:
                if config.success:
                    feature_vector = self._extract_features(
                        config.model_size_mb, config.n_ctx,
                        config.available_gpu_memory_mb, config.n_gpu_layers
                    )
            ...
            for param_name in self.models.keys():
                ...
                # Scale target values
                target_scaled = self.scalers[param_name].fit_transform(target_values.reshape(-1, 1)).flatten()
        ...
        try:
            ...
            if attempts_file.exists():
                with open(attempts_file, 'r') as f:
                    data = json.load(f)
                    self.recovery_attempts = [OOMRecoveryAttempt(**item) for item in data]
                    
            self.logger.info(f"Loaded {len(self.configurations)} configurations and {len(self.recovery_attempts)} recovery attempts")
    ...
    def _save_training_data(self):
        ...
        try:
            ...
            with open(config_file, 'w') as f:
                json.dump([asdict(config) for config in self.configurations], f, indent=2)
                
            attempts_file = self.data_dir / "recovery_attempts.json"
            with open(attempts_file, 'w') as f:
                json.dump([asdict(attempt) for attempt in self.recovery_attempts], f, indent=2)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ml_models_trained": all(model is not None for model in self.models.values()) if HAS_SKLEARN else False
        }