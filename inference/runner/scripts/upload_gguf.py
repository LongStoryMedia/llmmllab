...
def setup_huggingface_auth(token: Optional[str] = None, token_file: Optional[str] = None) -> str:
    ...
    if token:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to read token from file {token_file}: {e}")
...
def upload_model_file(
    ...
    logger.info(
        f"Starting upload of {file_path} ({file_size_mb:.2f} MB) to {repo_id}/{path_in_repo}")
...
if __name__ == "__main__":
    ...
    # Optional arguments
    parser.add_argument(
        "--organization", help="Organization name for the repository")
    parser.add_argument("--private", action="store_true",
                        help="Make the repository private")
    parser.add_argument("--description", help="Description of the model")
    parser.add_argument(
        "--model-name", help="Name for the model (defaults to repo name)")
    parser.add_argument("--token", help="Hugging Face token")
    parser.add_argument(
        "--token-file", help="Path to file containing Hugging Face token")
    parser.add_argument("--model-card-template",
                        help="Path to a custom model card template file")
