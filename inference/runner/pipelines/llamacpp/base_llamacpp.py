...
class BaseLlamaCppPipeline(BasePipeline):
    ...
    def initialize_llama_with_optimization(self) -> llama_cpp.Llama:
        ...
        if (
            ...
            self._logger.info(
                "🎯 Parameter optimization enabled, finding optimal values..."
            )
            try:
                ...
                self._logger.info(
                    f"✅ Parameter optimization complete: {optimized_params}"
                )
            except Exception as e:
                self._logger.warning(
                    f"Parameter optimization failed, using base params: {e}"
                )
    ...
    def _initialize_llama(
        ...
        if force_params:
            ...
            self._logger.info(
                f"🔍 DEBUG Using force_params: n_ctx={current_params.n_ctx}, n_batch={current_params.n_batch}"
            )
        else:
        ...
        while attempt <= max_attempts:
            ...
            try:
                ...
                # Set a process-level timeout to prevent hanging indefinitely
                import signal

                timeout_seconds = (
                    crash_prevention.timeout_seconds if crash_prevention else 300
                )

                def timeout_handler(signum, frame):
                    raise TimeoutError(
                        f"Model initialization timed out after {timeout_seconds} seconds"
                    )
                ...
                # Clear timeout alarm on any exception
                try:
                    signal.alarm(0)
                    signal.signal(signal.SIGALRM, old_handler)
                except:
                ...
                is_oom = any(
                    ...
                    for t in [
                        ...
                        "resource temporarily unavailable",  # System resource limits
                        "killed",  # Process killed by OOM killer
                        "segmentation fault",  # Memory corruption crashes
                        "aborted",  # Process aborts
                        "timeout",  # Our custom timeout
                        "timed out",  # Various timeout errors
                        "bad_alloc",  # C++ memory allocation failures
                        "std::bad_alloc",  # Standard C++ allocation error
                        "cannot allocate memory",  # System memory errors
                    ]
                )
    ...
    def _parse_tool_calls_from_content(
        ...
        # If no tool calls found with structured formats, try natural language parsing
        # This handles multimodal models that generate tool descriptions in plain text
        if not tool_calls:
            # Pattern: "- Tool call: `tool_name` with query `"parameter_value"`"
            # or "- Tool call: `tool_name` to fetch..."
            nl_pattern = r'- Tool call: `([^`]+)`\s+with\s+query\s+`"([^"]+)"`'
            
            matches = re.finditer(nl_pattern, content, re.IGNORECASE)
            ...
            for match in matches:
                try:
                    ...
                    if self._bound_tools and tool_name.lower() not in [
                        t.name.lower() for t in self._bound_tools
                    ]:
                        self._logger.warning(
                            f"Unrecognized tool name in natural language: {tool_name}, skipping."
                        )
                        continue
                    
                    # Create tool call with appropriate arguments
                    if tool_name.lower() in ["web_search", "search"]:
                        args = {"query": query}
                    elif tool_name.lower() in ["summarization", "summarize"]:
                        args = {"content": query}
                    elif tool_name.lower() in ["read_web_content", "read_content"]:
                        args = {"url": query}
                    else:
                        # Generic fallback - use query as main parameter
                        args = {"query": query}
                    ...
                    # Remove this natural language tool call from content
                    cleaned_content = cleaned_content.replace(match.group(0), "").strip()
                    
                    self._logger.info(
                        f"✅ Parsed natural language tool call: {tool_name} with args {args}"
                    )
                    
                except Exception as e:
                    self._logger.warning(
                        f"Failed to parse natural language tool call: {e}, match: {match.group(0)}"
                    )
                    continue
            
            # Also try a more flexible pattern: "Tool call: `tool_name` to do something"
            if not tool_calls:
                flexible_pattern = r'Tool call: `([^`]+)`'
                matches = re.finditer(flexible_pattern, content, re.IGNORECASE)
                ...
                for match in matches:
                    try:
                        ...
                        # For tools without explicit parameters, try to extract from context
                        line_context = content[max(0, match.start()-100):match.end()+200]
                        
                        # Look for quoted strings that might be parameters
                        param_patterns = [
                            r'"([^"]{5,})"',  # Look for substantial quoted strings
                            r'`"([^"]+)"`',   # Backtick-quoted strings
                        ]
                        
                        args = {}
                        for pattern in param_patterns:
                            param_matches = re.findall(pattern, line_context)
                            if param_matches:
                                param_value = param_matches[0]  # Use first substantial match
                                
                                # Map to appropriate argument based on tool
                                if tool_name.lower() in ["web_search", "search"]:
                                    args = {"query": param_value}
                                elif tool_name.lower() in ["summarization", "summarize"]:
                                    args = {"content": param_value}
                                elif tool_name.lower() in ["read_web_content", "read_content"]:
                                    args = {"url": param_value}
                                else:
                                    args = {"query": param_value}
                                break
                        
                        # Only add if we found meaningful arguments
                        if args:
                            tool_call = LangChainToolCall(
                                id=f"call_{len(tool_calls)}",
                                name=tool_name,
                            ...
                            self._logger.info(
                                f"✅ Parsed flexible natural language tool call: {tool_name} with args {args}"
                            )
                    
                    except Exception as e:
                        self._logger.warning(
                            f"Failed to parse flexible natural language tool call: {e}"
                        )
                        continue
    ...
    def close(self):
        ...
        try:
            ...
            if hasattr(self, "profile") and self.profile:
                ...
                if success:
                    self._logger.debug(
                        f"🔓 Unlocked pipeline for model: {self.profile.model_name}"
                    )
                else:
                    self._logger.debug(
                        f"🔓 Pipeline unlock skipped for model: {self.profile.model_name} (likely remote)"
                    )
        except Exception as e:
