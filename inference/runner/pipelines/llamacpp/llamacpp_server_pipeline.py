...
class LlamaCppServerManager:
    ...
    def _build_server_args(self) -> List[str]:
        ...
        args.extend(
            ["-c", str(params.num_ctx or 90000)]
        )  # Use short form like your working example with larger context
        args.extend(["--batch-size", str(params.batch_size or 256)])
        args.extend(
            ["-ub", str(params.batch_size or 256)]
        )  # Use short form like working example
        ...
        if (
            ...
            and self.model.task
            and "embed" in str(self.model.task).lower()
        ):
    ...
    def _wait_for_server(self) -> bool:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        self._logger.debug(
                            f"Model still loading - /health returns 503: {message}"
                        )
                    except:
        ...
        self._logger.error(
            f"Server/model failed to load within {self._startup_timeout} seconds"
        )
        return False
...
class LlamaCppServerPipeline(BasePipeline):
    ...
    def _initialize_persistent_server(self):
        ...
        try:
            ...
            self.server_manager = LlamaCppServerManager(
                model=self.model, profile=self.profile, user_config=self.user_config
            )
            ...
            logger.info(
                f"Persistent server ready at {base_url} - model loaded and cached",
                component="LlamaCppServerPipeline",
                model=self.model.name,
            )
    ...
    def _initialize_openai_client(self) -> None:
        ...
        try:
            ...
            self._logger.info(
                f"OpenAI client initialized for server at {self.server_manager.server_url}"
            )
