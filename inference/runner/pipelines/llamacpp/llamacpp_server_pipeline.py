...
class LlamaCppServerPipeline(BasePipeline):
                    ...
                    if hasattr(tool, "args_schema") and tool.args_schema:
                        try:
                            ...
                            if "properties" in full_schema:
                                ...
                                for param_name, param_schema in full_schema[
                                    "properties"
                                ].items():
                                    ...
                                    if param_name not in ["runtime"]:
                                        ...
                                        if param_name in full_schema.get(
                                            "required", []
                                        ):
        ...
        for msg in messages:
            if isinstance(msg, HumanMessage):
                ...
                if hasattr(msg, "tool_calls") and msg.tool_calls:
                    ...
                    self._logger.debug(
                        f"🔄 Converted {len(openai_tool_calls)} tool calls to OpenAI format"
                    )
    ...
    def bind_tools(self, tools: List[Any], **kwargs: Any) -> "LlamaCppServerPipeline":
        ...
        if self._bound_tools:
            ...
            if new_tools:
                self._logger.debug(
                    f"Adding {len(new_tools)} new tools, skipping {len(tools) - len(new_tools)} duplicates"
                )
                self._bound_tools.extend(new_tools)
            else:
                self._logger.debug(
                    f"All {len(tools)} tools already bound, skipping duplicates"
                )
        else:
