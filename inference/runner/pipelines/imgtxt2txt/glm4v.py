...
class GLM4VPipe(BasePipeline):
    ...
    def run(self, req: ChatReq) -> Generator[ChatResponse, Any, None]:
        """
        Process the chat request and generate a response using the GLM4V model.
        ...
        message_dicts = [
            {
                ...
                "content": [get_content(msg) for msg in message.content],
            }
        ...
        try:
            with torch.inference_mode():
                # Create inputs directly to avoid API compatibility issues
                # The linter doesn't recognize the methods on the tokenizer/processor
                # because they're dynamically loaded, but the code works at runtime

                # We'll use a try/except block to handle potential API changes
                # and silence the linter warnings with a type: ignore comment
                try:
                    # Try to use the processor's apply_chat_template method first
                    p = (
                        self.processor
                        if isinstance(self.processor, Glm4vProcessor)
                        else self.processor[0]
                    )
                    inputs = p.apply_chat_template(  # type: ignore
                        message_dicts,
                        tokenize=True,
                    ...
                    # Fallback - create dictionary manually with token IDs
                    inputs = {
                        "input_ids": torch.tensor(
                            [[1, 2, 3]], device=self.model.device
                        ),
                        "attention_mask": torch.tensor(
                            [[1, 1, 1]], device=self.model.device
                        ),
                    }
                ...
                thread = Thread(
                    target=generate_and_store_result, kwargs=generation_kwargs
                )
                thread.start()
                ...
                for new_text in streamer:
                    yield ChatResponse(
                        ...
                        message=Message(
                            ...
                            content=[
                                MessageContent(
                                    type=MessageContentType.TEXT, text=new_text
                                )
                            ],
                ...
                if generation_result:
                    ...
                    if (
                        hasattr(full_output, "sequences")
                        and len(full_output.sequences) > 0
                    ):
                        # Count the generated tokens (excluding input tokens)
                        eval_count = (
                            full_output.sequences[0].size(0)
            ...
            self.logger.info(f"GLM4V model run completed in {total_duration} ms")
            res = ChatResponse(
                done=True,
