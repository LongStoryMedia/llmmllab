...
const ImagePage = () => {
    ...
    const [inferenceSteps, setInferenceSteps] = useState(DEFAULT_SETTINGS.inference_steps);
    const [guidanceScale, setGuidanceScale] = useState(DEFAULT_SETTINGS.guidance_scale);
    ...
    return (_jsxs(Box, { sx: { p: 3, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }, children: [_jsx(Box, { sx: { width: { xs: '100%', md: '40%', lg: '30%' } }, children: _jsx(ImageGallery, { images: images, selectedImage: selectedImage, onSelectImage: handleSelectImage, onDeleteImage: handleDeleteImage, onDownloadImage: handleDownload }) }), _jsx(Box, { sx: { width: { xs: '100%', md: '60%', lg: '70%' } }, children: _jsx(ImageForm, { selectedImage: selectedImage, selectedImageData: selectedImageData, prompt: prompt, setPrompt: setPrompt, negativePrompt: negativePrompt, setNegativePrompt: setNegativePrompt, width: width, setWidth: setWidth, height: height, setHeight: setHeight, inferenceSteps: inferenceSteps, setInferenceSteps: setInferenceSteps, guidanceScale: guidanceScale, setGuidanceScale: setGuidanceScale, selectedModel: selectedModel, setSelectedModel: setSelectedModel, isGenerating: isGenerating, onGenerateImage: handleGenerateImage, onEditImage: handleEditImage, onCancelEdit: () => setSelectedImage(null) }) })] }));
};
