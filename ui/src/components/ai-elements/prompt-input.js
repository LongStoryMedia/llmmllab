...
export function PromptInputAttachments({ children, className, ...props }) {
    ...
    return (_jsx("div", { className: cn("flex flex-wrap items-center gap-2 p-3", className), ...props, children: attachments.files.map((file) => (_jsx(Fragment, { children: children(file) }, file.id))) }));
}
...
export const PromptInput = ({ className, accept, multiple, globalDrop, syncHiddenInput, maxFiles, maxFileSize, onError, onSubmit, children, ...props }) => {
    ...
    const inner = (_jsxs(_Fragment, { children: [_jsx("span", { "aria-hidden": "true", className: "hidden", ref: anchorRef }), _jsx("input", { accept: accept, "aria-label": "Upload files", className: "hidden", multiple: multiple, onChange: handleChange, ref: inputRef, title: "Upload files", type: "file" }), _jsx("form", { className: cn("w-full", className), onSubmit: handleSubmit, ...props, children: _jsx(InputGroup, { className: "overflow-hidden", children: children }) })] }));
