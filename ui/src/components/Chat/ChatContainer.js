...
const ScrollableArea = styled('div')(({ $autoPosition }) => ({
    flex: 1,
    overflowY: 'auto',
    overflowX: 'hidden',
    padding: '1rem',
...
const ChatContainer = memo(({ messages, streamingMessage }) => {
    ...
    return (_jsx(ChatContainerWrapper, { children: _jsx(ScrollableArea, { ref: scrollableAreaRef, "$autoPosition": shouldUseAutoPosition, className: "chat-scrollable-area", children: _jsx(Conversation, { children: _jsx(ConversationContent, { children: allMessages.map((message, index) => (_jsx(ChatBubble, { message: message }, message.id || `message-${index}`))) }) }) }) }));
});
