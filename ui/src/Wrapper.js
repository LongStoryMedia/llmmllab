...
import { CssBaseline, styled, GlobalStyles, useTheme } from '@mui/material';
...
const adjustLightness = (hsl, adjustment) => {
    const [h, s, l] = hsl.split(' ').map((v, i) => i === 2 ? Math.max(0, Math.min(100, parseInt(v) + adjustment)) : v);
    return `${h} ${s} ${l}%`;
};
...
const Wrapper = () => {
    ...
    return (_jsxs(ThemeProvider, { theme: themeMode, children: [_jsx(CssBaseline, {}), _jsx(GlobalStyles, { styles: globalStyles }), _jsx(ConfigProvider, { children: auth.evaluating ? (_jsx(LoadingContainer, { children: _jsx(LoadingAnimation, { size: 1000 }) })) : (_jsx(ChatProvider, { children: _jsx(MainLayout, { children: _jsx(Router, {}) }) })) })] }));
};
