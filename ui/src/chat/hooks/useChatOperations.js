...
import { chat, getToken, cancel } from '../../api';
 ...
 */
export const useChatOperations = (state, actions) => {
    const auth = useAuth();
    ...
    const sendMessage = useCallback(async (message) => {
        ...
        try {
            ...
            let finalMessage;
            for await (const chunk of chat(getToken(auth.user), userMessage, abortController.current.signal)) {
            ...
            if (finalMessage) {
                ...
                actions.setMessages(prev => {
                    const filtered = prev.filter(msg => msg.role === MessageRoleValues.USER ||
     ...
     */
    const replayMessage = useCallback(async (message) => {
        ...
        try {
            ...
            abortController.current = new AbortController();
            for await (const chunk of replay(getToken(auth.user), conversationId, message, abortController.current.signal)) {
                const updatedState = streaming.processChunk(chunk);
    ...
    const saveEditAndReplay = useCallback(async (messageId, newContent) => {
    ...
    return {
        ...
        selectConversation: useCallback(async (id) => {
