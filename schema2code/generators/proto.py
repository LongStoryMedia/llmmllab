...
class ProtoGenerator:
    @staticmethod
    def generate(schema: Dict[str, Any],
                 ...
                 external_imports: Optional[List[str]] = None) -> str:
        ...
        if schema.get("properties"):
            for _, prop_schema in schema.get("properties", {}).items():
                ...
                if prop_schema.get("type") == "array" and prop_schema.get("items", {}).get("$ref"):
                    ...
                    if not item_ref.startswith("#"):  # External reference
                        ...
                        if file_name.endswith((".yaml", ".yml", ".json")):
                            ...
                            proto_import = f"{base_name}.proto"
                            imports.append(proto_import)
                    elif item_ref.startswith("#/definitions/"):  # Internal reference to definition
        ...
        # Handle root object type
            ...
            output.extend(ProtoGenerator._generate_message_type(
        ...
        if "definitions" in schema:
            for def_name, def_schema in schema.get("definitions", {}).items():
                ...
                if def_schema.get("type") == "object":
                    ...
                    output.extend(ProtoGenerator._generate_enum_type(def_schema, type_name))

        # Join all lines
        return "\n".join(output)

    @staticmethod
    def _get_required_imports(schema: Dict[str, Any], ref_resolver: Optional[SchemaRefResolver] = None) -> List[str]:
    ...
    @staticmethod
    def _generate_message_type(schema: Dict[str, Any], type_name: str, processed_types: Set[str],
                               ref_resolver: Optional[SchemaRefResolver] = None) -> List[str]:
        ...
        if schema.get("additionalProperties"):
            ...
            if schema.get("additionalProperties") is True:
                ...
                output.append(f"  map<string, string> additional_properties = {field_number};")
                field_number += 1
                ...
                if not add_props:
                    ...
                    output.append(f"  // Additional properties as key-value pairs")
                    output.append(f"  map<string, {value_type}> additional_properties = {field_number};")
                    field_number += 1
    ...
    @staticmethod
    def _get_field_type(schema: Dict[str, Any], ref_resolver: Optional[SchemaRefResolver],
                        processed_types: Set[str], prop_name: str) -> Tuple[str, bool, Optional[List[str]]]:
                ...
                if referred_schema:
                    if "enum" in referred_schema:
                        ...
                        if referred_schema.get("type") == "string":
                            ...
                            nested_type = ProtoGenerator._generate_enum_type(referred_schema, enum_name)
        ...
        if schema.get("type") == "object":
            ...
            if type_name not in processed_types:
                ...
                if has_properties:
                    ...
                    nested_type.append("  // This message intentionally left empty or will be extended later")
        ...
        if schema_type == "string":
            ...
            elif schema_format == "uri" or schema_format == "url" or schema_format == "email":
                return "string", is_repeated, nested_type
            else:
                return "string", is_repeated, nested_type
