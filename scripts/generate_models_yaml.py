...
and extracting metadata using gguf_dump.py.
...
def get_gguf_metadata(gguf_path: str) -> dict[str, Any]:
    ...
    venv_python = "/opt/venv/shared/bin/python"
...
"""
    ...
    if result.returncode == 0:
        ...
        print(f"Warning: Failed to extract metadata: {result.stderr}", file=sys.stderr)
        return {}
...
def detect_model_task(metadata: dict[str, Any]) -> str:
    ...
    # Text-to-text (LLM)
    llm_archs = {'llama', 'llava', 'phi3', 'gemma', 'mistral', 'qwen', 'deepseek',
                 'stablelm', 'opt', 'bloom', 'falcon', 'gpt2', 'gptj', 'gptneox',
                 'mpt', 'codegen', 'refact', 'nemotron', 'granite', 'gemma2',
                 'command-r', 'dbrx', 'exaone', 'grit', 'kunyu', 'minicpm', 'olmo',
                 'openelm', 'orion', 'persimmon', 'phi3small', 'phi4', 'qwen2',
                 'qwen3', 'starcoder', 'jamba', 'mixtral', 'yi', 'chatglm',
                 'internlm', 'baichuan', 'qwen2.5', 'llama3', 'llama3.1', 'llama3.2'}

    # Text-to-image (SDXL, SD3, etc.)
    img2img_archs = {'stable-diffusion', 'sdxl', 'sd3', 'flux', 'pixtral', 'kandinsky',
                     'controlnet', 'inpaint', 'upscaler'}

    # Image-to-text (vision models)
    vision_archs = {'llava', 'idefics', 'paligemma', 'instructblip', 'vision-encoder',
                    'clip'}
...
def create_model_definition(file_path: str, metadata: dict[str, Any], mmproj_map: dict[str, str]) -> dict[str, Any]:
    ...
    arch = metadata.get('general.architecture', metadata.get('llama.architecture', '')).lower()
    ...
    basename = metadata.get('general.basename', os.path.basename(file_path).replace('.gguf', ''))

    # Extract quantization level from file name if not in metadata
    if not quantization:
        filename = os.path.basename(file_path)
        # Pattern: match any Q or IQ pattern (handles q8_0, iq4_xs, q6_k_s, q8_k_xl, etc.)
        quant_match = re.search(r'^(ud-)?([a-z]*[QI][a-z0-9_]+)\.gguf', filename, re.IGNORECASE)
        if quant_match:
    ...
    context_length = (
        ...
        metadata.get('context_length', 2048)
    )
    ...
    specialization = 'Text'
    if 'mmproj' in file_path.lower() or 'clip' in metadata.get('general.description', '').lower() or metadata.get('general.type') == 'mmproj':
        specialization = 'Vision'
    elif 'embed' in file_path.lower() or 'embedding' in metadata.get('general.description', '').lower():
    ...
    details: dict[str, Any] = {
        ...
        'weight': 1.0,
        'size': file_size,
        'original_ctx': int(context_length) if isinstance(context_length, (int, float)) else 2048,
    }
    ...
    n_heads = (
        ...
        metadata.get('phi3.attention.head_count') or
        metadata.get('qwen3next.attention.head_count') or
        metadata.get('qwen35moe.attention.head_count') or
        metadata.get('clip.vision.attention.head_count') or
        metadata.get('nomic-bert-moe.attention.head_count')
    )
    if n_heads:
    ...
    n_kv_heads = (
        metadata.get('llama.attention.head_count_kv') or
        metadata.get('phi3.attention.head_count_kv') or
        metadata.get('qwen3next.attention.head_count_kv') or
        metadata.get('qwen35moe.attention.head_count_kv') or
        metadata.get('clip.vision.attention.head_count_kv') or
        metadata.get('nomic-bert-moe.attention.head_count_kv')
    )
    if n_kv_heads:
...
def models_to_yaml(models: list[dict[str, Any]]) -> str:
    ...
    for model in models:
        ...
        lines.append("      gguf_file: " + escape_yaml_value(details.get('gguf_file', '')))
        ...
        lines.append("      parameter_size: " + escape_yaml_value(details.get('parameter_size', '')))

        if 'quantization_level' in details:
            lines.append("      quantization_level: " + escape_yaml_value(str(details.get('quantization_level', ''))))
        if 'precision' in details:
            lines.append("      precision: " + escape_yaml_value(details.get('precision', '')))
        if 'dtype' in details:
            lines.append("      dtype: " + escape_yaml_value(details.get('dtype', '')))
        if 'specialization' in details:
            lines.append("      specialization: " + escape_yaml_value(details.get('specialization', '')))
        ...
        if 'clip_model_path' in details and details['clip_model_path']:
            lines.append("      clip_model_path: " + escape_yaml_value(details.get('clip_model_path', '')))
        ...
        if 'description' in details and details['description']:
            lines.append("      description: " + escape_yaml_value(str(details.get('description', ''))))
...
def main():
    ...
    for i, gguf_path in enumerate(gguf_files, 1):
        ...
        if args.verbose:
            print(f"  -> Model: {model_def['name']}, Task: {model_def['task']}, Pipeline: {model_def['pipeline']}")
